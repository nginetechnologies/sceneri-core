...
#include <TestFramework/Renderer/RenderInstances.h>
...
namespace JPH::DebugRendering
{
	...
	public:
		...
		/// Constructor
		DebugRendererImp(Renderer* inRenderer, const Font* inFont, const ngine::IO::ConstZeroTerminatedPathView assetsDirectory);
	...
	private:
		...
		public:
			...
			BatchImpl(Renderer* inRenderer, D3D_PRIMITIVE_TOPOLOGY inType)
				: RenderPrimitive(inRenderer, inType)
			{
			}
		...
		struct InstanceWithLODInfo : public Instance
		{
			/// Constructor
			InstanceWithLODInfo(
				Mat44Arg inModelMatrix, Mat44Arg inModelMatrixInvTrans, ColorArg inModelColor, const AABox& inWorldSpaceBounds, float inLODScaleSq
			)
		...
		struct Instances
		{
			...
			/// Start index in mInstancesBuffer for each of the LOD in the geometry pass. Length is one longer than the number of LODs to indicate
			/// how many instances the last lod has.
			Array<int> mGeometryStartIdx;

			/// Start index in mInstancesBuffer for each of the LOD in the light pass. Length is one longer than the number of LODs to indicate
			/// how many instances the last lod has.
