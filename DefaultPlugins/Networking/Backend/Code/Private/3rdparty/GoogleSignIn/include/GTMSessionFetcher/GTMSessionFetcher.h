...
#if !defined(GTM_BACKGROUND_TASK_FETCHING) && (TARGET_OS_IOS || TARGET_OS_TV || TARGET_OS_MACCATALYST)
...
typedef void (^GTMSessionFetcherCompletionHandler)(NSData *_Nullable data, NSError *_Nullable error);
...
typedef void (^GTMSessionFetcherChallengeBlock)(GTMSessionFetcher *fetcher, NSURLAuthenticationChallenge *challenge,
...
typedef void (^GTMSessionFetcherWillRedirectBlock)(NSHTTPURLResponse *redirectResponse, NSURLRequest *redirectRequest,
                                                   GTMSessionFetcherWillRedirectResponse response);
typedef void (^GTMSessionFetcherAccumulateDataBlock)(NSData *_Nullable buffer);
typedef void (^GTMSessionFetcherSimulateByteTransferBlock)(NSData *_Nullable buffer, int64_t bytesWritten,
                                                           int64_t totalBytesWritten,
                                                           int64_t totalBytesExpectedToWrite);
typedef void (^GTMSessionFetcherReceivedProgressBlock)(int64_t bytesWritten, int64_t totalBytesWritten);
typedef void (^GTMSessionFetcherDownloadProgressBlock)(int64_t bytesWritten, int64_t totalBytesWritten,
                                                       int64_t totalBytesExpectedToWrite);
typedef void (^GTMSessionFetcherSendProgressBlock)(int64_t bytesSent, int64_t totalBytesSent,
                                                   int64_t totalBytesExpectedToSend);
typedef void (^GTMSessionFetcherWillCacheURLResponseResponse)(NSCachedURLResponse *_Nullable cachedResponse);
...
typedef void (^GTMSessionFetcherTestResponse)(NSHTTPURLResponse *_Nullable response, NSData *_Nullable data,
...
typedef void (^GTMFetcherDecoratorFetcherWillStartCompletionHandler)(NSURLRequest *_Nullable, NSError *_Nullable);
...
__deprecated_msg("implement GTMSessionFetcherAuthorizer instead") @protocol GTMFetcherAuthorizationProtocol<NSObject>
@required
...
- (void)authorizeRequest:(nullable NSMutableURLRequest *)request
       ...
       didFinishSelector:(SEL)sel __deprecated_msg("implement authorizeRequest:completionHandler: instead");
...
- (void)authorizeRequest:(nullable NSMutableURLRequest *)request delegate:(id)delegate didFinishSelector:(SEL)sel;
...
//
// This is called on the callback queue.
@property (atomic, copy, nullable) GTMSessionFetcherWillCacheURLResponseBlock willCacheURLResponseBlock;
...
//
// This is called on the callback queue.
@property (atomic, copy, nullable) GTMSessionFetcherMetricsCollectionBlock metricsCollectionBlock API_AVAILABLE(
...
- (void)setProperty:(nullable id)obj forKey:(NSString *)key; // Pass nil for obj to remove the property.
...
- (BOOL)waitForCompletionWithTimeout:(NSTimeInterval)timeoutInSeconds __deprecated_msg("Use XCTestExpectation instead");
...
#define GTMSessionCheckSynchronized(obj)                                                                               \
    {                                                                                                                  \
        GTMSESSION_ASSERT_DEBUG([GTMSessionSyncMonitorInternal functionsHoldingSynchronizationOnObject:obj],           \
