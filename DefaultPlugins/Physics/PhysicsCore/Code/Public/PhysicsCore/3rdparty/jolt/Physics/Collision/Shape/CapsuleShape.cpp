...
MassProperties CapsuleShape::GetMassProperties(const float density) const
{
	MassProperties p;
...
) const
{
	...
	inRenderer->DrawCapsule(
		inCenterOfMassTransform * Mat44::sScale(inScale.Abs().GetX()),
...
void CapsuleShape::CollidePoint(
...
void CapsuleShape::GetTrianglesStart(
...
) const
{
	...
	Mat44 top_matrix = world_matrix *
	                   Mat44(Vec4(mRadius, 0, 0, 0), Vec4(0, mRadius, 0, 0), Vec4(0, 0, mRadius, 0), Vec4(0, mHalfHeightOfCylinder, 0, 1));
	...
	Mat44 bottom_matrix =
		world_matrix * Mat44(Vec4(mRadius, 0, 0, 0), Vec4(0, mRadius, 0, 0), Vec4(0, 0, mRadius, 0), Vec4(0, -mHalfHeightOfCylinder, 0, 1));
	context->AddPart(bottom_matrix, sCapsuleBottomTriangles.data(), sCapsuleBottomTriangles.size());
}

int CapsuleShape::GetTrianglesNext(
