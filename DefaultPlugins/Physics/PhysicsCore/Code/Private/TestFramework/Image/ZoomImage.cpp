...
#include <TestFramework/Image/ZoomImage.h>
#include <TestFramework/Image/Surface.h>
#include <TestFramework/Image/BlitSurface.h>
...
namespace JPH::DebugRendering
{
	...
	{
		return mFilter == inRHS.mFilter && mWrapFilter == inRHS.mWrapFilter && mBlur == inRHS.mBlur;
	}
	...
	static void sPrecalculateFilter(
		const ZoomSettings& inZoomSettings, int inOldLength, int inNewLength, int inOffsetFactor, Array<Array<Contrib>>& outContrib
	)
	{
		...
		// Get scale
		float scale = float(inNewLength) / (float)inOldLength;
		...
		for (int i = 0; i < inNewLength; ++i)
		{
			...
			for (int source = left; source <= right; ++source)
			{
				...
				c.mWeight = int(4096.0f * filter.GetValue(fscale * (center - (float)source)));
				if (c.mWeight == 0)
					continue;
	...
	bool ZoomImage(RefConst<Surface> inSrc, Ref<Surface> ioDst, const ZoomSettings& inZoomSettings)
	{
		...
		// Determine the temporary format that will require the least amount of components to be zoomed and the least amount of bytes pushed
		// around
		...
		else
		{
			...
			float operations_vh = float(dst->GetWidth()) *
			                      (filter.GetSupport() * (float)src->GetHeight() + filter.GetSupport() * (float)dst->GetHeight());
			float operations_hv = float(dst->GetHeight()) *
			                      (filter.GetSupport() * (float)src->GetWidth() + filter.GetSupport() * (float)dst->GetWidth());
