...
{ 
	...
	const BodyView bodies = mBodyManager->GetBodies();
	JPH_ASSERT(mMaxBodies == mBodyManager->GetMaxBodies());

	LayerState *state = new LayerState [mNumLayers];

	// Sort bodies on layer
	Body * const * const bodies_ptr = bodies.GetData(); // C pointer or else sort is incredibly slow in debug mode
	...
	while (b_start < b_end)
	{
		...
		BroadPhaseLayer::Type broadphase_layer = (BroadPhaseLayer::Type)bodies[*b_start]->GetBroadPhaseLayer();
		...
		{
			JPH_ASSERT(bodies[*b]->GetID() == *b, "Provided BodyID doesn't match BodyID in body manager");
			JPH_ASSERT(!bodies[*b]->IsInBroadPhase());
			Tracking &t = mTracking[b->GetIndex()];
			...
			t.mObjectLayer = bodies[*b]->GetObjectLayer();
		}
...
{ 
	...
	for (BroadPhaseLayer::Type broadphase_layer = 0; broadphase_layer < mNumLayers; broadphase_layer++)
	{
		...
		if (l.mBodyStart != nullptr)
		{
			...
			{
				JPH_ASSERT(bodies[*b]->GetID() == *b, "Provided BodyID doesn't match BodyID in body manager");
				JPH_ASSERT(mTracking[b->GetIndex()].mBroadPhaseLayer == broadphase_layer);
				JPH_ASSERT(mTracking[b->GetIndex()].mObjectLayer == bodies[*b]->GetObjectLayer());
				JPH_ASSERT(!bodies[*b]->IsInBroadPhase());
				bodies[*b]->SetInBroadPhaseInternal(true);
			}
		}
	}
...
{ 
	...
	JPH_IF_ENABLE_ASSERTS(const BodyView bodies = mBodyManager->GetBodies();)
	JPH_ASSERT(mMaxBodies == mBodyManager->GetMaxBodies());
...
{ 
	...
	while (b_start < b_end)
	{
		...
		for (const BodyID *b = b_start; b < b_mid; ++b)
		{
			...
			JPH_ASSERT(bodies[*b]->IsInBroadPhase());
			bodies[*b]->SetInBroadPhaseInternal(false);
		}
...
void BroadPhaseQuadTree::NotifyBodiesLayerChanged(BodyID *ioBodies, int inNumber)
{
	...
	for (BodyID *body_id = ioBodies + inNumber - 1; body_id >= ioBodies; --body_id)
	{
		...
		JPH_ASSERT(bodies[*body_id]->GetID() == *body_id, "Provided BodyID doesn't match BodyID in body manager");
