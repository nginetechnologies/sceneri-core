...
public:
	...
	inline RefTarget(const RefTarget&)
	{ /* Do not copy refcount */
	}
	inline ~RefTarget()
	{
		JPH_IF_ENABLE_ASSERTS(uint32 value = mRefCount.load(memory_order_relaxed);) JPH_ASSERT(value == 0 || value == cEmbedded);
	} ///< assert no one is referencing us

	/// Mark this class as embedded, this means the type can be used in a compound or constructed on the stack.
	/// The Release function will never destruct the object, it is assumed the destructor will be called by whoever allocated
	/// the object and at that point in time it is checked that no references are left to the structure.
	inline void SetEmbedded() const
	{
		JPH_IF_ENABLE_ASSERTS(uint32 old =) mRefCount.fetch_add(cEmbedded, memory_order_relaxed);
		JPH_ASSERT(old < cEmbedded);
	}

	/// Assignment operator
	inline RefTarget& operator=(const RefTarget&)
	{ /* Don't copy refcount */
		return *this;
	}
...
public:
	...
	inline Ref(const Ref<T>& inRHS)
		: mPtr(inRHS.mPtr)
	{
		AddRef();
	}
	inline Ref(Ref<T>&& inRHS) noexcept
		: mPtr(inRHS.mPtr)
	{
		inRHS.mPtr = nullptr;
	}
	...
	inline Ref<T>& operator=(const Ref<T>& inRHS)
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	inline Ref<T>& operator=(Ref<T>&& inRHS) noexcept
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	/// Comparison
	inline bool operator==(const T* inRHS) const
	{
		return mPtr == inRHS;
	}
	inline bool operator==(const Ref<T>& inRHS) const
	{
...
public:
	...
	inline RefConst(const T* inRHS)
		: mPtr(inRHS)
	{
		AddRef();
	}
	inline RefConst(const RefConst<T>& inRHS)
		: mPtr(inRHS.mPtr)
	{
		AddRef();
	}
	inline RefConst(RefConst<T>&& inRHS) noexcept
		: mPtr(inRHS.mPtr)
	{
		inRHS.mPtr = nullptr;
	}
	inline RefConst(const Ref<T>& inRHS)
		: mPtr(inRHS.mPtr)
	{
		AddRef();
	}
	inline RefConst(Ref<T>&& inRHS) noexcept
		: mPtr(inRHS.mPtr)
	{
		inRHS.mPtr = nullptr;
	}
	...
	/// Assignment operators
	inline RefConst<T>& operator=(const T* inRHS)
	{
		if (mPtr != inRHS)
		{
			Release();
	...
	inline RefConst<T>& operator=(const RefConst<T>& inRHS)
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	inline RefConst<T>& operator=(RefConst<T>&& inRHS) noexcept
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	inline RefConst<T>& operator=(const Ref<T>& inRHS)
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	inline RefConst<T>& operator=(Ref<T>&& inRHS) noexcept
	{
		if (mPtr != inRHS.mPtr)
		{
			Release();
	...
	inline bool operator==(const RefConst<T>& inRHS) const
	{
		return mPtr == inRHS.mPtr;
	}
	inline bool operator==(const Ref<T>& inRHS) const
	...
	void** InternalGetPointer()
	{
		return const_cast<void**>(reinterpret_cast<const void**>(&mPtr));
	}
private:
	/// Use "variable = nullptr;" to release an object, do not call these functions
	inline void AddRef()
