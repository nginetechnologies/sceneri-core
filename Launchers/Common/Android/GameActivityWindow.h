...
{
	enum class InputTypeFlags : uint32
	{
		/**
	...
	ENUM_FLAG_OPERATORS(InputTypeFlags);

	struct GameActivityWindow
	{
		GameActivityWindow(android_app* pAndroidApp);

		void OnWindowRecreated(android_app* pAndroidApp);

		void SetWindow(Rendering::Window& window);
		[[nodiscard]] Optional<Rendering::Window*> GetWindow() const
		{
			return m_pWindow;
		}

		ANativeWindow* GetNativeWindow() const;
		Math::Vector2ui GetClientAreaSize() const;
		float GetDotsPerInch() const;

		void ShowVirtualKeyboard(const EnumFlags<InputTypeFlags> inputTypeFlags) const;
		void HideVirtualKeyboard() const;

		using SignInWithGoogleCallback = Function<void(const ConstStringView userIdentifier, const ConstStringView idToken), 24>;
		void SignInWithGoogleCachedCredentials(SignInWithGoogleCallback&& callback);
		void SignInWithGoogle(SignInWithGoogleCallback&& callback);
		void SignOutOfAllProviders() const;

		void OnReceivedDeepLink(const ConstStringView link);

		SignInWithGoogleCallback m_signInWithGoogleCallback;
	private:
		android_app* m_pApplication{nullptr};
		GameActivity* const m_pGameActivity{nullptr};
		ANativeWindow* m_pNativeWindow{nullptr};
		Optional<Rendering::Window*> m_pWindow;
		float m_dotsPerInch{0.f};
	};
}
