...
Font::Font(DebugRendering::Renderer* inRenderer, const ngine::IO::ConstZeroTerminatedPathView assetsDirectory)
...
bool Font::Create(const char* inFontName, int inCharHeight)
{
	...
	Ref<DebugRendering::SoftwareSurface> surface =
		new DebugRendering::SoftwareSurface(mHorizontalTexels, mVerticalTexels, DebugRendering::ESurfaceFormat::L8);
	surface->Clear();
	surface->Lock(DebugRendering::ESurfaceLockMode::Write);
...
try_again:;
	...
	for (int c = cBeginChar + 1; c < cEndChar; ++c)
	{
		...
		if (int(x + widths[c].abcB + cSpacingH) > mHorizontalTexels)
		{
			...
			if (y + textmetric.tmHeight + cSpacingV > mVerticalTexels)
			{
				...
				// Create new surface
				surface->UnLock();
				surface = new DebugRendering::SoftwareSurface(mHorizontalTexels, mVerticalTexels, DebugRendering::ESurfaceFormat::L8);
				surface->Clear();
				surface->Lock(DebugRendering::ESurfaceLockMode::Write);
	...
	// Load vertex shader
	ComPtr<ID3DBlob> vtx =
		mRenderer->CreateVertexShader(IO::Path::Combine(IO::Path(mAssetsDirectory), MAKE_PATH("Shaders/FontVertexShader.hlsl")));

	// Load pixel shader
	ComPtr<ID3DBlob> pix =
		mRenderer->CreatePixelShader(IO::Path::Combine(IO::Path(mAssetsDirectory), MAKE_PATH("Shaders/FontPixelShader.hlsl")));
	...
	mPipelineState = mRenderer->CreatePipelineState(
		...
		DebugRendering::PipelineState::EDepthTest::Off,
		DebugRendering::PipelineState::EBlendMode::AlphaTest,
		DebugRendering::PipelineState::ECullMode::Backface
	);
...
bool Font::CreateString(
	Mat44Arg inTransform, const string_view& inText, ColorArg inColor, DebugRendering::RenderPrimitive& ioPrimitive
) const
{
...
void Font::DrawText3D(Mat44Arg inTransform, const string_view& inText, ColorArg inColor) const
{
	...
	DebugRendering::RenderPrimitive primitive(mRenderer, D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
