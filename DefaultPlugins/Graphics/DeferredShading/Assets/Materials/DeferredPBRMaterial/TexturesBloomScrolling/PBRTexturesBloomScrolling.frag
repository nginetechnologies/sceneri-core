...
#endif
{
	layout(offset = 16) float emissive;
...
void main()
{
	...
	mat3 TBN = mat3(T, B, N);
	const vec3 sampledNormal = texture(sampler2D(normalTexture, normalSampler), coordinates).xyz * 2.0 - vec3(1.0);
	vec3 tnorm = (TBN * normalize(sampledNormal));
	outNormal.xyz = tnorm*0.5+0.5;

	const float metallic = texture(sampler2D(metallicTexture, metallicSampler), coordinates).r;
	const float roughness = texture(sampler2D(roughnessTexture, roughnessSampler), coordinates).r;
	outMaterialProperties.xyzw = vec4(metallic, roughness, pushConstants.emissive, 1.0);
