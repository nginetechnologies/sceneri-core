...
{
	DebugRendererImp::DebugRendererImp(Renderer* inRenderer, const Font* inFont, const ngine::IO::ConstZeroTerminatedPathView assetsDirectory)
		...
		, mFont(inFont)
	{
		...
		// Lines
		ComPtr<ID3DBlob> vtx_line =
			mRenderer->CreateVertexShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/LineVertexShader.hlsl")));
		ComPtr<ID3DBlob> pix_line =
			mRenderer->CreatePixelShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/LinePiXELShader.hlsl")));
		mLineState = mRenderer->CreatePipelineState(
			vtx_line.Get(),
			line_vertex_desc,
			ARRAYSIZE(line_vertex_desc),
			pix_line.Get(),
		...
		// Triangles
		ComPtr<ID3DBlob> vtx_triangle =
			mRenderer->CreateVertexShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/TriangleVertexShader.hlsl")));
		ComPtr<ID3DBlob> pix_triangle =
			mRenderer->CreatePixelShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/TrianglePixelShader.hlsl")));
		mTriangleStateBF = mRenderer->CreatePipelineState(
			vtx_triangle.Get(),
			triangles_vertex_desc,
			ARRAYSIZE(triangles_vertex_desc),
			pix_triangle.Get(),
			D3D12_FILL_MODE_SOLID,
		...
		// Shadow pass
		ComPtr<ID3DBlob> vtx_shadow =
			mRenderer->CreateVertexShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/TriangleDepthVertexShader.hlsl")));
		ComPtr<ID3DBlob> pix_shadow =
			mRenderer->CreatePixelShader(IO::Path::Combine(IO::Path(assetsDirectory), MAKE_PATH("Shaders/TriangleDepthPixelShader.hlsl")));
	...
	DebugRenderer::Batch
	DebugRendererImp::CreateTriangleBatch(const Vertex* inVertices, int inVertexCount, const uint32* inIndices, int inIndexCount)
	{
