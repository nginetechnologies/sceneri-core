...
#include "PhysicsCore/Components/Constraints/DistanceConstraintComponent.h"
...
{
	DistanceConstraint::DistanceConstraint(const DistanceConstraint& templateComponent, const Cloner& cloner)
	...
	DistanceConstraint::DistanceConstraint(Initializer&& initializer)
		: Entity::Component3D(Forward<BaseType::Initializer>(initializer))
	{
	}

	DistanceConstraint::DistanceConstraint(const Deserializer& deserializer)
	...
	void DistanceConstraint::OnCreated()
	{
	...
	void DistanceConstraint::OnDestroy()
	{
		Assert(m_constraintIdentifier.IsInvalid());
	}

	void DistanceConstraint::OnEnable()
	{
		if (!GetRootScene().IsTemplate())
		{
			CreateConstraint();
	...
	void DistanceConstraint::OnDisable()
	{
	...
	void DistanceConstraint::OnWorldTransformChanged(const EnumFlags<Entity::TransformChangeFlags> flags)
	{
	...
	void DistanceConstraint::CreateConstraint()
	{
		Physics::Data::Scene& physicsScene = *GetRootSceneComponent().FindDataComponentOfType<Physics::Data::Scene>();
		...
		JPH::DistanceConstraintSettings* pSettings = new JPH::DistanceConstraintSettings();
		...
		pSettings->mMinDistance = pSettings->mMaxDistance =
			(constrainedWorldTransform.GetLocation() - constraintWorldTransform.GetLocation()).GetLength();
	...
	void DistanceConstraint::SetConstrainedComponent(Entity::Component3DPicker constrained)
	{
		m_constrainedComponent = constrained;
		CreateConstraint();
	}

	Entity::Component3DPicker DistanceConstraint::GetConstrainedComponent() const
	{
	...
	Optional<Entity::Component3D*> DistanceConstraint::GetBodyComponent() const
	{
	...
	[[maybe_unused]] const bool wasDistanceConstraintRegistered =
		Entity::ComponentRegistry::Register(UniquePtr<Entity::ComponentType<DistanceConstraint>>::Make());
	[[maybe_unused]] const bool wasDistanceConstraintTypeRegistered = Reflection::Registry::RegisterType<DistanceConstraint>();
}
