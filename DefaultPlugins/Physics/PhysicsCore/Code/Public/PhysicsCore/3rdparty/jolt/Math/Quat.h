...
public:
	...
	inline explicit Quat(Vec4Arg inV)
		: mValue(inV)
	{
	}

	JPH_INLINE Quat(const ngine::Math::Quaternionf& inQ)
		: Quat(inQ.x, inQ.y, inQ.z, inQ.w)
	{
	}
	[[nodiscard]] JPH_INLINE operator ngine::Math::Quaternionf() const
	{
	...
	inline bool operator==(QuatArg inRHS) const
	{
	...
	inline bool IsNormalized(float inTolerance = 1.0e-3f) const
	{
	...
	JPH_INLINE Vec4Arg GetXYZW() const
	{
		return mValue;
	}
	...
	/// @return [0, 0, 0, 0]
	JPH_INLINE static Quat sZero()
	{
		return Quat(Vec4::sZero());
	}

	/// @return [1, 0, 0, 0] (or in storage format Quat(0, 0, 0, 1))
	JPH_INLINE static Quat sIdentity()
	{
		return Quat(0, 0, 0, 1);
	}
	...
	{
		return Quat(mValue + inRHS.mValue);
	}
	JPH_INLINE Quat operator-(QuatArg inRHS) const
	{
		return Quat(mValue - inRHS.mValue);
	...
	inline friend Quat operator*(float inValue, QuatArg inRHS)
	{
		return Quat(inRHS.mValue * inValue);
	}
	JPH_INLINE Quat operator/(float inValue) const
	{
		return Quat(mValue / inValue);
	...
	{
		return mValue.Dot(inRHS.mValue);
	}
	...
	/// Ensures that the W component is positive by negating the entire quaternion if it is not. This is useful when you want to store a
	/// quaternion as a 3 vector by discarding W and reconstructing it as sqrt(1 - x^2 - y^2 - z^2).
	JPH_INLINE Quat EnsureWPositive() const
	{
		return Quat(Vec4::sXor(mValue, Vec4::sAnd(mValue.SplatW(), UVec4::sReplicate(0x80000000).ReinterpretAsFloat())));
	}
	...
	/// Get rotation angle around inAxis (uses Swing Twist Decomposition to get the twist quaternion and uses q(axis, angle) = [cos(angle /
	/// 2), axis * sin(angle / 2)])
	JPH_INLINE float GetRotationAngle(Vec3Arg inAxis) const
	{
		return GetW() == 0.0f ? JPH_PI : 2.0f * ATan(GetXYZ().Dot(inAxis) / GetW());
	}
	...
	/// To String
	friend ostream& operator<<(ostream& inStream, QuatArg inQ)
	{
		inStream << inQ.mValue;
		return inStream;
	}
