...
#include "PhysicsCore/Components/Constraints/LineConstraintComponent.h"
...
{
	LineConstraint::LineConstraint(const LineConstraint& templateComponent, const Cloner& cloner)
	...
	LineConstraint::LineConstraint(Initializer&& initializer)
		: Entity::Component3D(Forward<BaseType::Initializer>(initializer))
	{
	}

	LineConstraint::LineConstraint(const Deserializer& deserializer)
	...
	void LineConstraint::OnDestroy()
	{
		Assert(m_constraintIdentifier.IsInvalid());
	}

	void LineConstraint::OnEnable()
	{
		if (!GetRootScene().IsTemplate())
		{
			CreateConstraint();
	...
	void LineConstraint::OnWorldTransformChanged(const EnumFlags<Entity::TransformChangeFlags> flags)
	{
	...
	void LineConstraint::CreateConstraint()
	{
		Physics::Data::Scene& physicsScene = *GetRootSceneComponent().FindDataComponentOfType<Physics::Data::Scene>();
		...
		JPH::SliderConstraintSettings* pSettings = new JPH::SliderConstraintSettings();
		...
		pSettings->mSliderAxis1 = constraintParentBodyWorldTransform.InverseTransformDirection(constraintWorldTransform.GetForwardColumn());
		pSettings->mNormalAxis1 = constraintParentBodyWorldTransform.InverseTransformDirection(constraintWorldTransform.GetUpColumn());

		pSettings->mSliderAxis2 = constrainedWorldTransform.InverseTransformDirection(constraintWorldTransform.GetForwardColumn());
	...
	void LineConstraint::SetConstrainedComponent(Entity::Component3DPicker constrained)
	{
		m_constrainedComponent = constrained;
		CreateConstraint();
	}

	Entity::Component3DPicker LineConstraint::GetConstrainedComponent() const
	{
	...
	Optional<Entity::Component3D*> LineConstraint::GetBodyComponent() const
	{
	...
	[[maybe_unused]] const bool wasLineConstraintRegistered =
		Entity::ComponentRegistry::Register(UniquePtr<Entity::ComponentType<LineConstraint>>::Make());
	[[maybe_unused]] const bool wasLineConstraintTypeRegistered = Reflection::Registry::RegisterType<LineConstraint>();
}
