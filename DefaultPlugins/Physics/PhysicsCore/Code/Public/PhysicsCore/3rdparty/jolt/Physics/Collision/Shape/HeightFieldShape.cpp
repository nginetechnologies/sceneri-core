...
void HeightFieldShape::CalculateActiveEdges()
{
	...
	for (uint y = 0; y < count_min_1; ++y)
		for (uint x = 0; x < count_min_1; ++x)
		{
			...
			bool edge1_active = y == count_min_1 - 1 ||
			                    ActiveEdges::IsEdgeActive(normals[offset], normals[offset + 2 * count_min_1 + 1], x2y2 - x1y2);
...
void HeightFieldShape::StoreMaterialIndices(const Array<uint8>& inMaterialIndices)
{
	...
	mMaterialIndices.resize(
		((Square(count_min_1) * mNumBitsPerMaterialIndex + 7) >> 3) + 1
	...
	, mMaterials(inSettings.mMaterials)
{
	...
	if (min_value <= max_value) // Only when there was collision
	{
		...
		// We want to be able to exactly quantize min_value (this is important in case the heightfield is entirely flat) so we subtract that
		...
		for (uint x = 0; x < n; ++x)
		{
			...
			uint max_bx =
				x == n - 1
					? mBlockSize
					: mBlockSize +
							1; // for interior blocks take 1 more because the triangles connect to the next block so we must include their height too
...
inline void
HeightFieldShape::sGetRangeBlockOffsetAndStride(uint inNumBlocks, uint inMaxLevel, uint& outRangeBlockOffset, uint& outRangeBlockStride)
{
	outRangeBlockOffset = sGridOffsets[inMaxLevel - 1];
	outRangeBlockStride = inNumBlocks >> 1;
}

inline void HeightFieldShape::GetBlockOffsetAndScale(
	uint inBlockX, uint inBlockY, uint inRangeBlockOffset, uint inRangeBlockStride, float& outBlockOffset, float& outBlockScale
) const
{
...
MassProperties HeightFieldShape::GetMassProperties([[maybe_unused]] const float density) const
{
...
void HeightFieldShape::SetMaterial(const SubShapeID& inSubShapeID, const PhysicsMaterial* pMaterial)
{
	if (mMaterials.empty())
		return;
	if (mMaterials.size() == 1)
	{
		mMaterials[0] = pMaterial;
	}
	...
	uint count_min_1 = mSampleCount - 1;
	JPH_ASSERT(x < count_min_1);
	JPH_ASSERT(y < count_min_1);

	// Calculate at which bit the material index starts
	uint bit_pos = (x + y * count_min_1) * mNumBitsPerMaterialIndex;
	uint byte_pos = bit_pos >> 3;
	bit_pos &= 0b111;
	...
	// Return the material
	mMaterials[material_index] = pMaterial;
}
...
) const
{
	...
	if (mGeometry.empty())
	{
		...
		for (uint32 by = 0; by < mSampleCount; by += block_size)
			for (uint32 bx = 0; bx < mSampleCount; bx += block_size)
			{
				...
				// Create batch
				if (num_triangles > 0)
					mGeometry.push_back(new DebugRenderer::Geometry(
						inRenderer->CreateTriangleBatch(triangles),
	...
	DebugRenderer::ECullMode cull_mode = ScaleHelpers::IsInsideOut(inScale) ? DebugRenderer::ECullMode::CullFrontFace
	...
	if (sDrawTriangleOutlines)
	{
		struct Visitor
		{
			...
			JPH_INLINE int VisitRangeBlock(
				...
				UVec4& ioProperties,
				[[maybe_unused]] int inStackTop
			) const
			{
				UVec4 valid = UVec4::sOr(
					UVec4::sOr(Vec4::sLess(inBoundsMinX, inBoundsMaxX), Vec4::sLess(inBoundsMinY, inBoundsMaxY)),
					Vec4::sLess(inBoundsMinZ, inBoundsMaxZ)
				);
...
public:
	...
	JPH_INLINE void WalkHeightField(Visitor& ioVisitor)
	{
		...
		do
		{
			...
			if (level >= max_level)
			{
				...
				int num_results = ioVisitor.VisitRangeBlock(
					transposed_min.GetColumn4(0),
					transposed_min.GetColumn4(1),
					transposed_min.GetColumn4(2),
					transposed_max.GetColumn4(0),
					transposed_max.GetColumn4(1),
					transposed_max.GetColumn4(2),
					colliding_blocks,
					mTop
				);
			...
			else
			{
				...
				UVec4 internal_cell_size =
					UVec4::sReplicate(block_size << (max_level - level - 1)); // subtract 1 from level because we have an internal grid of 2x2
	...
	// This can be used to have the visitor early out (ioVisitor.ShouldAbort() returns true) and later continue again (call WalkHeightField()
	// again)
...
bool HeightFieldShape::CastRay(const RayCast& inRay, const SubShapeIDCreator& inSubShapeIDCreator, RayCastResult& ioHit) const
{
	...
	struct Visitor
	{
		JPH_INLINE explicit Visitor(
			const HeightFieldShape* inShape, const RayCast& inRay, const SubShapeIDCreator& inSubShapeIDCreator, RayCastResult& ioHit
		)
			: mHit(ioHit)
...
) const
{
	...
	{
		JPH_INLINE explicit Visitor(
			const HeightFieldShape* inShape,
			const RayCast& inRay,
			const RayCastSettings& inRayCastSettings,
			const SubShapeIDCreator& inSubShapeIDCreator,
			CastRayCollector& ioCollector
...
void HeightFieldShape::CollidePoint(
...
struct HeightFieldShape::HSGetTrianglesContext
{
	...
	{
		// When the buffer is full and we cannot process the triangles, abort the height field walk. The next time GetTrianglesNext is called we
		// will continue here.
...
void HeightFieldShape::GetTrianglesStart(
...
int HeightFieldShape::GetTrianglesNext(
...
void HeightFieldShape::sCollideConvexVsHeightField(
	...
	[[maybe_unused]] const ShapeFilter& inShapeFilter
)
{
	JPH_PROFILE_FUNCTION();
...
void HeightFieldShape::sCollideSphereVsHeightField(
	const Shape* inShape1,
	const Shape* inShape2,
	Vec3Arg inScale1,
	Vec3Arg inScale2,
...
{
	return Stats(
		sizeof(*this) + mMaterials.size() * sizeof(Ref<PhysicsMaterial>) + mRangeBlocks.size() * sizeof(RangeBlock) +
			mHeightSamples.size() * sizeof(uint8) + mActiveEdges.size() * sizeof(uint8) + mMaterialIndices.size() * sizeof(uint8),
