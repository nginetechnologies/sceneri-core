...
BodyID BodyInterface::AcquireBodyIdentifier()
{
	return mBodyManager->AcquireBodyIdentifier();
}

void BodyInterface::ReturnBodyIdentifier(const BodyID identifier)
{
	mBodyManager->ReturnBodyIdentifier(identifier);
}

BodyID::IndexType BodyInterface::GetMaximumUsedBodyCount() const
{
	return mBodyManager->GetMaximumUsedBodyCount();
}

Body& BodyInterface::CreateBody(const BodyID identifier, const BodyCreationSettings& inSettings)
{
	return mBodyManager->CreateBody(identifier, inSettings);
}
...
bool BodyInterface::IsBodyValid(const BodyID& inBodyID) const
{
	return mBodyLockInterface->TryGetBody(inBodyID) != nullptr;
}
...
void BodyInterface::CreateAndAddBody(const BodyID identifier, const BodyCreationSettings& inSettings, EActivation inActivationMode)
{
	CreateBody(identifier, inSettings);
	AddBody(identifier, inActivationMode);
}
...
void BodyInterface::ActivateAllBodies()
{
	BodyLockInterface::MutexMask mutexMask = mBodyManager->GetAllBodiesMutexMask();
	mBodyLockInterface->LockWrite(mutexMask);

	BodyIDVector bodies;
	mBodyManager->GetBodyIDs(bodies);
	bodies.erase(
		std::remove_if(
			bodies.begin(),
			bodies.end(),
			[this](const JPH::BodyID bodyIdentifier)
			{
				JPH::Body* pBody = mBodyManager->TryGetBody(bodyIdentifier);
				return pBody == nullptr || !pBody->IsInBroadPhase();
			}
		),
		bodies.end()
	);
	...
	if (bodies.size() > 0)
	{
		mBodyManager->ActivateBodies(bodies.data(), (int)bodies.size());
	}

	mBodyLockInterface->UnlockWrite(mutexMask);
}
...
void BodyInterface::DeactivateAllBodies()
{
	BodyLockInterface::MutexMask mutexMask = mBodyManager->GetAllBodiesMutexMask();
	mBodyLockInterface->LockWrite(mutexMask);

	BodyIDVector bodies;
	mBodyManager->GetActiveBodies(bodies);
	if (bodies.size() > 0)
	{
		mBodyManager->DeactivateBodies(bodies.data(), (int)bodies.size());
	}
...
TwoBodyConstraint*
BodyInterface::CreateConstraint(const TwoBodyConstraintSettings* inSettings, const BodyID& inBodyID1, const BodyID& inBodyID2)
{
...
void BodyInterface::NotifyShapeChanged(
...
void BodyInterface::SetPositionAndRotationWhenChanged(
...
void BodyInterface::MoveKinematic(const BodyID& inBodyID, Vec3Arg inTargetPosition, float inDeltaTime)
{
	BodyLockWrite lock(*mBodyLockInterface, inBodyID);
	if (lock.Succeeded())
	{
		...
		body.MoveKinematic(inTargetPosition, inDeltaTime);
...
void BodyInterface::MoveKinematic(const BodyID& inBodyID, QuatArg inTargetRotation, float inDeltaTime)
{
	BodyLockWrite lock(*mBodyLockInterface, inBodyID);
	if (lock.Succeeded())
	{
		...
		body.MoveKinematic(inTargetRotation, inDeltaTime);
...
void BodyInterface::SetPositionRotationAndVelocity(
