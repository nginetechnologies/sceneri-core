...
TaperedCapsuleShapeSettings::TaperedCapsuleShapeSettings(
	float inHalfHeightOfTaperedCylinder, float inTopRadius, float inBottomRadius, const PhysicsMaterial* inMaterial
)
...
MassProperties TaperedCapsuleShape::GetMassProperties(const float density) const
{
	AABox box = GetInertiaApproximation();

	MassProperties p;
	p.SetMassAndInertiaOfSolidBox(box.GetSize(), density);
	return p;
}
...
Vec3 TaperedCapsuleShape::GetSurfaceNormal(const SubShapeID& inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	else
	{
		...
		// We know that the perpendicular has length 1 and that it needs a y component where tan(alpha) = y / 1 in order to align it to the
		// surface
...
) const
{
	if (mGeometry == nullptr)
	{
		...
		mGeometry = inRenderer->CreateTriangleGeometryForConvex(
			[support](Vec3Arg inDirection)
			{
