...
// Sanity checks
		...
		for (uint ip = 0; ip < n - 1; ++ip)
			for (uint iq = ip + 1; iq < n; ++iq)
			{
				...
				if (sweep > 4 && abs(outEigVal[ip]) + g == abs(outEigVal[ip]) && abs(outEigVal[iq]) + g == abs(outEigVal[iq]))
				{
					a(ip, iq) = 0.0f;
				}
...
#define JPH_EVS_ROTATE(a, i, j, k, l) g = a(i, j), h = a(k, l), a(i, j) = g - s * (h + g * tau), a(k, l) = h + s * (g - h * tau)

						uint j;
						for (j = 0; j < ip; ++j)
							JPH_EVS_ROTATE(a, j, ip, j, iq);
						for (j = ip + 1; j < iq; ++j)
							JPH_EVS_ROTATE(a, ip, j, j, iq);
						for (j = iq + 1; j < n; ++j)
							JPH_EVS_ROTATE(a, ip, j, iq, j);
						for (j = 0; j < n; ++j)
							JPH_EVS_ROTATE(outEigVec, j, ip, j, iq);
