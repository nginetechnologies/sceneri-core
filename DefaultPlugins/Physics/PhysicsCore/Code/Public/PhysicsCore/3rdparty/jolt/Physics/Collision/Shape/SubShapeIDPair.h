...
public:
	...
	{
		return HashBytes(this, sizeof(SubShapeIDPair));
	}
private:
	BodyID mBody1ID;
...
namespace std
{
	/// Declare std::hash for SubShapeIDPair, note that std::hash is platform dependent and we need this one to be consistent because we sort
	/// on it in the ContactConstraintManager
	template<>
