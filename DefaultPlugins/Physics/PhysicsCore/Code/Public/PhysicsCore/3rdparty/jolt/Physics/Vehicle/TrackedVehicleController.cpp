...
JPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(TrackedVehicleControllerSettings
    ...
    JPH_ADD_ATTRIBUTE(TrackedVehicleControllerSettings, mEngine) JPH_ADD_ATTRIBUTE(TrackedVehicleControllerSettings, mTransmission)
...
WheelTV::WheelTV(WheelSettingsTV& inSettings)
	: Wheel(inSettings)
{
}
...
void WheelTV::Update(float inDeltaTime, const VehicleConstraint& inConstraint)
{
	...
	if (mContactBody != nullptr)
	{
		...
		mCombinedLongitudinalFriction = sqrt(settings->mLongitudinalFriction * contactFriction);
		mCombinedLateralFriction = sqrt(settings->mLateralFriction * contactFriction);
	}
	else
	{
		// No collision
...
void TrackedVehicleController::SyncLeftRightTracks()
{
	...
	{
		// Solve: (tl.mAngularVelocity + dl) / (tr.mAngularVelocity + dr) = mLeftRatio / mRightRatio and dl * tr.mInertia = -dr * tl.mInertia,
		// where dl/dr are the delta angular velocities for left and right tracks
		float impulse = (mLeftRatio * tr.mAngularVelocity - mRightRatio * tl.mAngularVelocity) /
		                (mLeftRatio * tr.mInertia + mRightRatio * tl.mInertia);
	...
	else
	{
		...
		// where dl/dr are the delta angular velocities for left and right tracks
		float impulse = (mLeftRatio * tr.mAngularVelocity - mRightRatio * tl.mAngularVelocity) /
		                (mRightRatio * tl.mInertia - mLeftRatio * tr.mInertia);
		tl.mAngularVelocity += impulse * tl.mInertia;
		tr.mAngularVelocity += impulse * tr.mInertia;
	}
}
...
void TrackedVehicleController::PostCollide(float inDeltaTime, PhysicsSystem& inPhysicsSystem)
{
	...
	if (transmission_torque != 0.0f)
	{
		...
		for (uint i = 0; i < size(mTracks); ++i)
		{
			...
			float track_max_angular_velocity = mEngine.GetCurrentRPM() /
			                                   (transmission_ratio * t.mDifferentialRatio * ratio * VehicleEngine::cAngularVelocityToRPM) *
			                                   1.001f;
		...
		if (brake_torque > 0.0f)
		{
			...
			if (total_radius > 0.0f)
			{
				...
				for (uint wheel_index : t.mWheels)
				{
					...
					if (w->HasContact())
					{
						// Impulse: p = F * dt = Torque / Wheel_Radius * dt, Torque = Total_Torque * Wheel_Radius / Summed_Radius => p = Total_Torque *
						// dt / Summed_Radius
...
bool TrackedVehicleController::SolveLongitudinalAndLateralConstraints(float inDeltaTime)
{
	...
	for (Wheel* w_base : mConstraint.GetWheels())
		if (w_base->HasContact())
		{
			...
			{
				// Assume we want to apply an angular impulse that makes the delta velocity between track and ground zero in one time step,
				...
				min_longitudinal_impulse = max_longitudinal_impulse = w->GetLongitudinalLambda() +
				                                                      Sign(linear_impulse) *
				                                                        min(abs(linear_impulse), max_longitudinal_friction_impulse);
	...
	for (const Wheel* w_base : mConstraint.GetWheels())
	{
		...
		if (w->HasContact())
		{
			...
			DebugRenderer::sInstance
				->DrawText3D(w->GetContactPosition(), StringFormat("S: %.2f", (double)w->GetSuspensionLength()), Color::sWhite, 0.1f);
		}
		else
		{
			// Draw 'no hit'
