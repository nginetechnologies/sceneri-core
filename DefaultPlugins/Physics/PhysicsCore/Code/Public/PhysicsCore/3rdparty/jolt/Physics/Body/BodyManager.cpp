...
void BodyManager::Init(uint inNumBodyMutexes, const BroadPhaseLayerInterface& inLayerInterface)
{
	UniqueLock lock(mBodiesMutex, EPhysicsLockTypes::BodiesList);

	// Num body mutexes must be a power of two and not bigger than our MutexMask
	uint num_body_mutexes =
		Clamp<uint>(GetNextPowerOf2(inNumBodyMutexes == 0 ? 2 * thread::hardware_concurrency() : inNumBodyMutexes), 1, sizeof(MutexMask) * 8);
...
BodyManager::BodyStats BodyManager::GetBodyStats() const
{
	...
	stats.mMaxBodies = BodyID::MaximumCount;

	for (const Body* body : GetBodies())
...
Body& BodyManager::CreateBody(const BodyID identifier, const BodyCreationSettings& inBodyCreationSettings)
{
	...
	mNumBodies++;
	// Add body
	m_bodies[identifier] = body;
	m_bodiesMask.Set(identifier);
	return *body;
}
...
void BodyManager::DestroyBodies(const BodyID* inBodyIDs, int inNumber)
{
	...
	// Update cached number of bodies
	[[maybe_unused]] const BodyID::IndexType previousBodyCount = mNumBodies.fetch_sub((BodyID::IndexType)inNumber);
	JPH_ASSERT(previousBodyCount >= (BodyID::IndexType)inNumber);

	for (const BodyID *b = inBodyIDs, *b_end = inBodyIDs + inNumber; b < b_end; b++)
	{
		// Get body
		BodyID body_id = *b;
		Body* body = m_bodies[body_id];
		...
		m_bodies[body_id] = nullptr;

		m_bodiesMask.Clear(body_id);
...
void BodyManager::ActivateBodies(const BodyID* inBodyIDs, int inNumber)
{
	...
	for (const BodyID *b = inBodyIDs, *b_end = inBodyIDs + inNumber; b < b_end; b++)
	{
		...
		if (pBody != nullptr)
		{
			...
			if (!body.IsStatic() && body.mMotionProperties->mIndexInActiveBodies == Body::cInactiveIndex)
			{
				...
				m_activeBodies[mNumActiveBodies] = body_id;
				mNumActiveBodies++; // Increment atomic after setting the body ID so that PhysicsSystem::JobFindCollisions (which doesn't lock the
				                    // mActiveBodiesMutex) will only read valid IDs
...
void BodyManager::DeactivateBodies(const BodyID* inBodyIDs, int inNumber)
{
	...
	for (const BodyID *b = inBodyIDs, *b_end = inBodyIDs + inNumber; b < b_end; b++)
	{
		...
		if (pBody != nullptr)
		{
			...
			{
				BodyID::IndexType last_body_index = mNumActiveBodies - 1;
				if (body.mMotionProperties->mIndexInActiveBodies != last_body_index)
				{
					...
					BodyID last_body_id = m_activeBodies[last_body_index];
					m_activeBodies[(BodyID::IndexType)body.mMotionProperties->mIndexInActiveBodies] = last_body_id;

					// Update that body's index in the active list
					Body& last_body = *m_bodies[last_body_id];
...
void BodyManager::GetActiveBodies(BodyIDVector& outBodyIDs) const
{
	...
	outBodyIDs.assign((const BodyID*)m_activeBodies.begin(), (const BodyID*)m_activeBodies.begin() + mNumActiveBodies);
}
...
BodyManager::MutexMask BodyManager::GetMutexMask(const BodyMask& bodies) const
{
	JPH_ASSERT(sizeof(MutexMask) * 8 >= mBodyMutexes.GetNumMutexes(), "MutexMask must have enough bits");

	if (bodies.GetNumberOfSetBits() >= (int)mBodyMutexes.GetNumMutexes())
	{
		// Just lock everything if there are too many bodies
		return GetAllBodiesMutexMask();
	}
	...
	{
		MutexMask mask = 0;
		for (const typename BodyID::IndexType bodyIndex : bodies.GetSetBitsIterator())
		{
			uint32 index = mBodyMutexes.GetMutexIndex(bodyIndex);
			mask |= (MutexMask(1) << index);
		}
		return mask;
	}
...
void BodyManager::SaveState(StateRecorder& inStream) const
{
...

	{
		...
		BodyIDVector sorted_active_bodies((const BodyID*)m_activeBodies.begin(), (const BodyID*)m_activeBodies.begin() + mNumActiveBodies);
...
bool BodyManager::RestoreState(StateRecorder& inStream)
{
...

	{
		...
		for (BodyID *id = m_activeBodies.GetData(), *id_end = m_activeBodies.GetData() + mNumActiveBodies; id < id_end; ++id)
			m_bodies[*id]->mMotionProperties->mIndexInActiveBodies = Body::cInactiveIndex;

		QuickSort(m_activeBodies.GetData(), m_activeBodies.GetData() + mNumActiveBodies); // Sort for validation
		...
		{
			inStream.Read(*id);
			m_bodies[*id]->mMotionProperties->mIndexInActiveBodies = uint32(id - m_activeBodies.GetData());
		}
...
void BodyManager::Draw(
	const DrawSettings& inDrawSettings,
	const PhysicsSettings& inPhysicsSettings,
	DebugRenderer* inRenderer,
	const BodyDrawFilter* inBodyFilter
)
{
	JPH_PROFILE_FUNCTION();
	...
	for (const Body* body : GetBodies())
		...
		{
			JPH_ASSERT(m_bodies[body->GetID()] == body);
			...
			if (inDrawSettings.mDrawGetSupportFunction)
				body->mShape->DrawGetSupportFunction(
					inRenderer,
					body->GetCenterOfMassTransform(),
					Vec3::sReplicate(1.0f),
					color,
					inDrawSettings.mDrawSupportDirection
				);
			...
			if (inDrawSettings.mDrawMassAndInertia && body->IsDynamic())
			{
				...
				inRenderer->DrawWireBox(
					body->GetCenterOfMassTransform() * Mat44::sRotation(mp->GetInertiaRotation()),
					AABox(-0.5f * box_size, 0.5f * box_size),
					Color::sOrange
				);
			...
			if (inDrawSettings.mDrawSleepStats && body->IsDynamic() && body->IsActive())
			{
				...
				for (int i = 0; i < 3; ++i)
					inRenderer->DrawWireSphere(
						body->mMotionProperties->mSleepTestSpheres[i].GetCenter(),
						body->mMotionProperties->mSleepTestSpheres[i].GetRadius(),
						sleep_color
					);
			}
		}
...
{
	// If this is the first time we flip the collision cache invalid flag, we need to add it to an internal list to ensure we reset the flag
	// at the end of the physics update
...
void BodyManager::ValidateActiveBodyBounds()
{
	...
	for (BodyID *id = m_activeBodies.GetData(), *id_end = m_activeBodies.GetData() + mNumActiveBodies; id < id_end; ++id)
	{
		...
		[[maybe_unused]] AABox cached = body->GetWorldSpaceBounds();
		[[maybe_unused]] AABox calculated = body->GetShape()->GetWorldSpaceBounds(body->GetCenterOfMassTransform(), Vec3::sReplicate(1.0f));
