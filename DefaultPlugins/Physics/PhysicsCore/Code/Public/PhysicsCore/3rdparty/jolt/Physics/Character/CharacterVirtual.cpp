...
CharacterVirtual::CharacterVirtual(
	const CharacterVirtualSettings* inSettings, Vec3Arg inPosition, QuatArg inRotation, PhysicsSystem* inSystem
)
...
void CharacterVirtual::sFillContactProperties(
	Contact& outContact, const Body& inBody, const taCollector& inCollector, const CollideShapeResult& inResult
)
{
...
void CharacterVirtual::CheckCollision(
	Vec3Arg inPosition,
	QuatArg inRotation,
...
) const
{
	...
	// Collide shape
	mSystem->GetNarrowPhaseQuery().CollideShape(
		inShape,
...
void CharacterVirtual::GetContactsAtPosition(
	Vec3Arg inPosition,
	Vec3Arg inMovementDirection,
	const Shape* inShape,
	TempContactList& outContacts,
	const BroadPhaseLayerFilter& inBroadPhaseLayerFilter,
	const ObjectLayerFilter& inObjectLayerFilter,
	const BodyFilter& inBodyFilter
) const
...
{
	// Only use this algorithm if we're penetrating further than this (due to numerical precision issues we can always penetrate a little bit
	// and we don't want to discard contacts if they just have a tiny penetration) We do need to account for padding (see
	// GetContactsAtPosition) that is removed from the contact distances, to compensate we add it to the cMinRequiredPenetration
...
bool CharacterVirtual::GetFirstContactForSweep(
	Vec3Arg inPosition,
...
) const
{
	...
	QuickSort(
		contacts.begin(),
		contacts.end(),
		[](const Contact& inLHS, const Contact& inRHS)
		{
			return inLHS.mFraction < inRHS.mFraction;
		}
	);
	...
	for (const Contact& c : contacts)
		if (c.mDistance + c.mNormal.Dot(inDisplacement) < -mCollisionTolerance && ValidateContact(c))
		{
			outContact = c;
			valid_contact = true;
			break;
	...
	// We want to maintain distance of cCharacterPadding (p) along plane normal outContact.mNormal (n) to the capsule by moving back along
	// inDisplacement (d) by amount d' cos(angle between d and -n) = -n dot d / |d| = p / d'
	// <=> d' = -p |d| / n dot d
		...
		if (mCosMaxSlopeAngle < 0.999f) // If cos(slope angle) is close to 1 then there's no limit
		{
			...
			if (dot > 0.0f && dot < mCosMaxSlopeAngle)
			{
				...
				vertical_constraint.mLinearVelocity =
					contact_velocity.Dot(normal) * normal; // Project the contact velocity on the new normal so that both planes push at an equal rate
				vertical_constraint.mPlane = Plane(
					normal,
					c.mDistance / normal.Dot(c.mNormal)
				); // Calculate the distance we have to travel horizontally to hit the contact plane
			}
		}
	}
}
...
void CharacterVirtual::SolveConstraints(
	Vec3Arg inVelocity,
	Vec3Arg inGravity,
	float inDeltaTime,
	float inTimeRemaining,
	ConstraintList& ioConstraints,
	IgnoredContactList& ioIgnoredContacts,
...
) const
{
	...
	for (uint iteration = 0; iteration < mMaxConstraintIterations; iteration++)
	{
		...
		QuickSort(
			...
			[](const Constraint* inLHS, const Constraint* inRHS)
			{
		...
		for (Constraint** c = previous_contacts.data(); c < previous_contacts.data() + num_previous_contacts; ++c)
			if (*c != constraint)
			{
				...
				{
					// We don't want parallel or anti-parallel normals as that will cause our cross product below to become zero. Slack is approx 10
					// degrees.
		...
		if (other_constraint != nullptr)
		{
			...
			// Cancel the constraint velocity in the other constraint plane's direction so that we won't try to apply it again and keep ping
			// ponging between planes
			constraint->mLinearVelocity -= min(0.0f, constraint->mLinearVelocity.Dot(other_normal)) * other_normal;

			// Cancel the other constraints velocity in this constraint plane's direction so that we won't try to apply it again and keep ping
			// ponging between planes
...
void CharacterVirtual::UpdateSupportingContact(TempAllocator& inAllocator)
{
	...
	for (const Contact& c : mActiveContacts)
		if (c.mHadCollision)
		{
			...
			if (cos_angle >= 0.08f)
			{
				...
				{
					// For keyframed objects that support us calculate the velocity at our position rather than at the contact position so that we
					// properly follow the object Note that we don't just take the point velocity because a point on an object with angular velocity
					// traces an arc, so if you just take point velocity * delta time you get an error that accumulates over time
	...
	{
		// If we're sliding we may actually be standing on multiple sliding contacts in such a way that we can't slide off, in this case we're
		// also supported
		...
		const JPH::Vec3 gravity = JPH::Vec3{0, 0, -9.81f};
		SolveConstraints(-mUp, gravity, 1.0f, 1.0f, constraints, ignored_contacts, time_simulated, displacement, inAllocator);
...
void CharacterVirtual::RefreshContacts(
	const BroadPhaseLayerFilter& inBroadPhaseLayerFilter,
	const ObjectLayerFilter& inObjectLayerFilter,
	const BodyFilter& inBodyFilter,
	TempAllocator& inAllocator
...
bool CharacterVirtual::SetShape(
	const Shape* inShape,
...
bool CharacterVirtual::WalkStairs(
	float inDeltaTime,
	Vec3Arg inGravity,
	Vec3Arg inStepUp,
...
)
{
	...
	if (sDrawWalkStairs)
	{
		...
		mShape->Draw(
			...
			GetCenterOfMassTransform(debug_pos, mRotation, mShape),
			Vec3::sReplicate(1.0f),
			Color::sYellow,
			false,
			true
		...
		&& contact.mNormal.Dot(mUp) < mCosMaxSlopeAngle) // Check slope angle
	{
		...
		bool hit = GetFirstContactForSweep(
			test_position,
			down,
			test_contact,
			dummy_ignored_contacts,
