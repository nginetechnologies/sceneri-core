...
JPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(WheeledVehicleControllerSettings
    ...
    JPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mEngine) JPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mTransmission)
      JPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mDifferentials)}

JPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(WheelSettingsWV
){JPH_ADD_ATTRIBUTE(WheelSettingsWV, mInertia) JPH_ADD_ATTRIBUTE(WheelSettingsWV, mAngularDamping)
    JPH_ADD_ATTRIBUTE(WheelSettingsWV, mMaxSteerAngle) JPH_ADD_ATTRIBUTE(WheelSettingsWV, mLongitudinalFriction)
      JPH_ADD_ATTRIBUTE(WheelSettingsWV, mLateralFriction) JPH_ADD_ATTRIBUTE(WheelSettingsWV, mMaxBrakeTorque)
...
WheelWV::WheelWV(WheelSettingsWV& inSettings)
	: Wheel(inSettings)
{
...
void WheelWV::Update(float inDeltaTime, const VehicleConstraint& inConstraint)
{
	...
	if (mContactBody != nullptr)
	{
		...
		mLongitudinalSlip = relative_longitudinal_velocity != 0.0f
		                      ? abs((mAngularVelocity * settings->mRadius - relative_longitudinal_velocity) / relative_longitudinal_velocity)
		                      : 0.0f;
		...
		mLateralSlip = relative_velocity_len < 1.0e-3f ? 0.0f
		                                               : RadiansToDegrees(ACos(abs(relative_longitudinal_velocity) / relative_velocity_len));
		float lateral_slip_friction = settings->mLateralFriction.GetValue(mLateralSlip);

		const float contactFriction = mContactBody->GetShape()->GetMaterial(mContactSubShapeID)->GetFriction();

		// Tire friction
		mCombinedLongitudinalFriction = sqrt(longitudinal_slip_friction * contactFriction);
		mCombinedLateralFriction = sqrt(lateral_slip_friction * contactFriction);
	}
	else
	{
		// No collision
		mLongitudinalSlip = 0.f;
		mLateralSlip = 0.f;
	...
	: VehicleController(inConstraint)
{
	...
	JPH_ASSERT(
		inSettings.mTransmission.mMode != ETransmissionMode::Auto || inSettings.mTransmission.mShiftUpRPM < inSettings.mEngine.mMaxRPM
	);
...
void WheeledVehicleController::PostCollide(float inDeltaTime, PhysicsSystem& inPhysicsSystem)
{
	...
	if (transmission_torque != 0.0f)
	{
		...
		float differential_max_angular_velocity = mEngine.GetCurrentRPM() / (transmission_ratio * VehicleEngine::cAngularVelocityToRPM) *
		                                          1.001f;
	...
	for (Wheel* w_base : mConstraint.GetWheels())
		if (w_base->HasContact())
		{
			...
			if (w->mBrakeImpulse != 0.0f)
			{
				...
				// Longitudinal impulse, note that we assume that once the wheels are locked that the brakes have more than enough torque to keep
				// the wheels locked so we exclude any rotation deltas
				impulse |= w->SolveLongitudinalConstraintPart(mConstraint, min_longitudinal_impulse, max_longitudinal_impulse);
			}
			else
			{
				// Assume we want to apply an angular impulse that makes the delta velocity between wheel and ground zero in one time step,
				// calculate the amount of linear impulse needed to do that
				...
				w->mMaxLongitudinalImpulse = min_longitudinal_impulse = max_longitudinal_impulse =
					w->GetLongitudinalLambda() + Sign(linear_impulse) * min(abs(linear_impulse), max_longitudinal_friction_impulse);
				...
				w->SetAngularVelocity(
					w->GetAngularVelocity() - (w->GetLongitudinalLambda() - prev_lambda) * settings->mRadius / settings->mInertia
				);
			}
		}
...
void WheeledVehicleController::Draw(DebugRenderer* inRenderer) const
{
	...
	mEngine
		.DrawRPM(inRenderer, rpm_meter_pos, rpm_meter_fwd, rpm_meter_up, mRPMMeterSize, mTransmission.mShiftDownRPM, mTransmission.mShiftUpRPM);

	// Draw current vehicle state
	String status = StringFormat(
		"Forward: %.1f, Right: %.1f, Brake: %.1f, HandBrake: %.1f\n"
		"Gear: %d, Clutch: %.1f, EngineRPM: %.0f, V: %.1f km/h",
		(double)mForwardInput,
		(double)mRightInput,
		(double)mBrakeInput,
		(double)mHandBrakeInput,
		mTransmission.GetCurrentGear(),
		(double)mTransmission.GetClutchFriction(),
		(double)mEngine.GetCurrentRPM(),
		(double)body->GetLinearVelocity().Length() * 3.6
	);
	...
	for (const Wheel* w_base : mConstraint.GetWheels())
	{
		...
		if (w->HasContact())
		{
			...
			inRenderer->DrawLine(
				...
				w->HasHitHardPoint() ? Color::sRed : Color::sGreen
			); // Red if we hit the 'max up' limit
			...
			DebugRenderer::sInstance->DrawText3D(
				w->GetContactPosition(),
				StringFormat("W: %.1f, S: %.2f", (double)w->GetAngularVelocity(), (double)w->GetSuspensionLength()),
				Color::sWhite,
				0.1f
			);
			DebugRenderer::sInstance->DrawText3D(
				w->GetContactPosition() - w->GetContactNormal() * 0.2f,
				StringFormat("FrLateral: %.2f (Slip: %.2f)", (double)w->mCombinedLateralFriction, (double)w->mLateralSlip),
				Color::sWhite,
				0.1f
			);
			DebugRenderer::sInstance->DrawText3D(
				w->GetContactPosition() - w->GetContactNormal() * 0.3f,
				StringFormat("FrLong: %.2f (Slip: %.2f)", (double)w->mCombinedLongitudinalFriction, (double)w->mLongitudinalSlip),
				Color::sWhite,
				0.1f
			);
			DebugRenderer::sInstance->DrawText3D(
				w->GetContactPosition() - w->GetContactNormal() * 0.4f,
				StringFormat("MaxImp: %.2f", (double)w->mMaxLongitudinalImpulse),
				Color::sWhite,
				0.1f
			);
		}
		else
		{
			...
			DebugRenderer::sInstance
				->DrawText3D(ws_position + max_droop, StringFormat("W: %.1f", (double)w->GetAngularVelocity()), Color::sRed, 0.1f);
		}
	}
}
