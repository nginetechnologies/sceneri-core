...
namespace JPH::DebugRendering
{
	...
	public:
		...
		void Initialize(ID3D12Device* inDevice)
		{
			...
			FatalErrorIfFailed(
				inDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, mCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&mCommandList))
			);
	...
	private:
		...
		UINT64 mFenceValue = 0;    ///< Current fence value, each time we need to wait we will signal the fence with this value, wait for it and
