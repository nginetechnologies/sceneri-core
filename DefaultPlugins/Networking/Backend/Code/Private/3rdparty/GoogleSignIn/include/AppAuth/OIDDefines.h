...
#define OID_UNAVAILABLE_USE_INITIALIZER(designatedInitializer)                                                         \
    {                                                                                                                  \
        NSString *reason =                                                                                             \
            [NSString stringWithFormat:@"Called: %@\nDesignated Initializer:%@", NSStringFromSelector(_cmd),           \
                                       NSStringFromSelector(designatedInitializer)];                                   \
        @throw [NSException exceptionWithName:@"Attempt to call unavailable initializer." reason:reason userInfo:nil]; \
    }
