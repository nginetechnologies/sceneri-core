...
DISABLE_CLANG_WARNING("-Wenum-enum-conversion")

PUSH_GCC_WARNINGS
DISABLE_GCC_WARNING("-Wunused-parameter");

PUSH_MSVC_WARNINGS
DISABLE_MSVC_WARNINGS(4245 4100 4242 5054)
...
enet_protocol_remove_sent_reliable_command(ENetPeer* peer, enet_uint16 reliableSequenceNumber, enet_uint8 channelID)
{
	...
	for (currentCommand = enet_list_begin(&peer->sentReliableCommands); currentCommand != enet_list_end(&peer->sentReliableCommands);
	...
	{
		for (currentCommand = enet_list_begin(&peer->outgoingCommands); currentCommand != enet_list_end(&peer->outgoingCommands);
		     currentCommand = enet_list_next(currentCommand))
		{
			outgoingCommand = (ENetOutgoingCommand*)currentCommand;
...
static int enet_protocol_check_outgoing_commands(ENetHost* host, ENetPeer* peer)
{
	...
	while (currentCommand != enet_list_end(&peer->outgoingCommands))
	{
		...
		if (outgoingCommand->command.header.command & ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE)
		{
			channel = outgoingCommand->command.header.channelID < peer->channelCount ? &peer->channels[outgoingCommand->command.header.channelID]
			                                                                         : NULL;
...
static int enet_protocol_send_outgoing_commands(ENetHost* host, ENetEvent* event, int checkForTimeouts)
{
	...
	while (host->continueSending)
		for (host->continueSending = 0, currentPeer = host->peers; currentPeer < &host->peers[host->peerCount]; ++currentPeer)
		{
			...
			{
				size_t originalSize = host->packetSize - sizeof(ENetProtocolHeader), compressedSize = host->compressor.compress(
																																							 host->compressor.context,
																																							 &host->buffers[1],
																																							 host->bufferCount - 1,
																																							 originalSize,
