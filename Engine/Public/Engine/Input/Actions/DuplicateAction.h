...
{
	struct DuplicateAction final : public Action
	{
		Function<void(), 24> OnDuplicate;

		void BindInputs(ActionMonitor& monitor, const Input::KeyboardDeviceType& keyboard)
		{
			m_inputIdentifierD = keyboard.GetInputIdentifier(Input::KeyboardInput::D);
			Action::BindInput(monitor, keyboard, m_inputIdentifierD);
		}

		void UnbindInputs(ActionMonitor& monitor, const Input::KeyboardDeviceType&)
		{
			Action::UnbindInput(monitor, m_inputIdentifierD);
		}
	protected:
		...
		) override
		{
			...
			if (inputIdentifier == m_inputIdentifierD && isBaseModifierActive)
			{
				OnDuplicate();
			}
		}
	...
	protected:
		InputIdentifier m_inputIdentifierD;
	};
}
