#include "TempVisualScripting/HealthCollectibleComponent.h"
#include "Tags.h"
...
{
	HealthCollectibleComponent::HealthCollectibleComponent(Initializer&& initializer)
	...
	HealthCollectibleComponent::HealthCollectibleComponent(const HealthCollectibleComponent& templateComponent, const Cloner& cloner)
		: SensorComponent(templateComponent, cloner)
		, m_healthReward(templateComponent.m_healthReward)
	...
	void HealthCollectibleComponent::OnCreated()
	{
		SensorComponent::OnCreated();
		OnComponentDetected.Add(*this, &HealthCollectibleComponent::Collect);
	...
	void HealthCollectibleComponent::Collect(SensorComponent&, Optional<Entity::Component3D*> pCollectorComponent)
	{
		const Optional<SceneRules*> pSceneRules = SceneRules::Find(GetRootSceneComponent());

		if (m_healthReward != 0 && pCollectorComponent.IsValid() && pSceneRules.IsValid())
		{
			if (Optional<Entity::SceneComponent*> pCollectorSceneComponent = pCollectorComponent->GetParentSceneComponent())
			{
				if (const Optional<HealthModule*> pHealthModule = pSceneRules->FindDataComponentOfType<HealthModule>())
				{
					if (pSceneRules->IsHost())
					{
						...
						{
							pHealthModule->AddHealth(*pSceneRules, pPlayerDataComponent->GetClientIdentifier(), m_healthReward);
						}
					}
				}
			}
		...
		if (pSceneRules.IsValid())
		{
			if (pSceneRules->IsHost())
			{
				...
				{
					pBoundComponent
						->BroadcastToAllClients<&HealthCollectibleComponent::ClientOnCollectedOnHost>(*this, GetSceneRegistry(), Network::Channel{0});
				}
			}
			else
			{
				...
				m_revertLocalCollectJobHandle = System::Get<Threading::JobManager>().ScheduleAsync(
					...
					{
						if (const Optional<HealthCollectibleComponent*> pCollectible = softComponentReference.Find<HealthCollectibleComponent>(sceneRegistry))
						{
	...
	void HealthCollectibleComponent::ClientOnCollectedOnHost(Network::Session::BoundComponent&, Network::LocalClient&)
	{
	...
	void HealthCollectibleComponent::OnCollectedInternal()
	{
	...
	void HealthCollectibleComponent::SetAudioAsset(const AudioAssetPicker asset)
	{
		if (asset.IsValid())
		{
	...
	HealthCollectibleComponent::AudioAssetPicker HealthCollectibleComponent::GetAudioAsset() const
	{
		return {m_audioAssetGuid, Audio::AssetFormat.assetTypeGuid};
	}

	void HealthCollectibleComponent::SetSceneAsset(const ScenePicker asset)
	{
	...
	HealthCollectibleComponent::ScenePicker HealthCollectibleComponent::GetSceneAsset() const
	{
		Entity::ComponentTemplateCache& sceneTemplateCache = System::Get<Entity::Manager>().GetComponentTemplateCache();
		return {
			Asset::Reference{
	...
	void HealthCollectibleComponent::SpawnMesh()
	{
	...
	Entity::Component3D& HealthCollectibleComponent::GetOwner()
	{
	...
	[[maybe_unused]] const bool wasHealthCollectibleRegistered =
		Entity::ComponentRegistry::Register(UniquePtr<Entity::ComponentType<HealthCollectibleComponent>>::Make());
	[[maybe_unused]] const bool wasHealthCollectibleTypeRegistered = Reflection::Registry::RegisterType<HealthCollectibleComponent>();
}
