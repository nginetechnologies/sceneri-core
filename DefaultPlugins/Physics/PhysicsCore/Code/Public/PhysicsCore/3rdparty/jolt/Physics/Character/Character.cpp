...
{
	return inLockBodies ? static_cast<const BodyLockInterface&>(inSystem->GetBodyLockInterface())
	                    : static_cast<const BodyLockInterface&>(inSystem->GetBodyLockInterfaceNoLock());
}
...
Character::Character(
	...
	const CharacterSettings* inSettings,
	Vec3Arg inPosition,
	QuatArg inRotation,
	uint64 inUserData,
	...
	, mLayer(inSettings->mLayer)
{
	...
	Body& body = mSystem->GetBodyInterface().CreateBody(bodyIdentifier, settings);
...
void Character::CheckCollision(
	const Shape* inShape, float inMaxSeparationDistance, CollideShapeCollector& ioCollector, bool inLockBodies
) const
{
...
void Character::PostSimulation(float inMaxSeparationDistance, bool inLockBodies)
{
	...
	public:
		// Constructor
		explicit MyCollector(Vec3Arg inGravity)
			: mGravity(inGravity)
		{
		}
	...
	// Collide shape
	auto getGravity = [this, inLockBodies]()
	{
		const JPH::BodyLockInterface& bodyLockInterface = sGetBodyLockInterface(mSystem, inLockBodies);
		BodyLockRead lock(bodyLockInterface, mBodyID);
		if (!lock.Succeeded())
			return JPH::Vec3{0, 0, 0};

		const Body& body = lock.GetBody();
		return body.GetGravity(*mSystem, bodyLockInterface, JPH::BroadPhaseLayer(), JPH::ObjectLayer());
	};

	const JPH::Vec3 gravity = getGravity();
	MyCollector collector(gravity);
