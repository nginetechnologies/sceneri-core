...
#include "PhysicsCore/Components/Constraints/PlaneConstraintComponent.h"
...
{
	PlaneConstraint::PlaneConstraint(const PlaneConstraint& templateComponent, const Cloner& cloner)
	...
	PlaneConstraint::PlaneConstraint(Initializer&& initializer)
		: Entity::Component3D(Forward<BaseType::Initializer>(initializer))
	{
	}

	PlaneConstraint::PlaneConstraint(const Deserializer& deserializer)
	...
	void PlaneConstraint::OnCreated()
	{
	...
	void PlaneConstraint::OnDestroy()
	{
		Assert(m_constraintIdentifier.IsInvalid());
	}

	void PlaneConstraint::OnEnable()
	{
		if (!GetRootScene().IsTemplate())
		{
			CreateConstraint();
	...
	void PlaneConstraint::OnDisable()
	{
	...
	void PlaneConstraint::OnWorldTransformChanged(const EnumFlags<Entity::TransformChangeFlags> flags)
	{
	...
	void PlaneConstraint::CreateConstraint()
	{
		Physics::Data::Scene& physicsScene = *GetRootSceneComponent().FindDataComponentOfType<Physics::Data::Scene>();
		...
		pSettings->mPosition1 = constraintParentBodyWorldTransform.InverseTransformLocation(constraintWorldTransform.GetLocation());
		pSettings->mPosition2 = constrainedWorldTransform.InverseTransformLocation(constraintWorldTransform.GetLocation());
		...
		// Lock the up axis
		pSettings->MakeFixedAxis(JPH::SixDOFConstraintSettings::EAxis::TranslationZ);

		// TODO: Expose to properties
		pSettings->MakeFixedAxis(JPH::SixDOFConstraintSettings::EAxis::RotationX);
		pSettings->MakeFixedAxis(JPH::SixDOFConstraintSettings::EAxis::RotationY);
		pSettings->MakeFixedAxis(JPH::SixDOFConstraintSettings::EAxis::RotationZ);
	...
	void PlaneConstraint::SetConstrainedComponent(Entity::Component3DPicker constrained)
	{
		m_constrainedComponent = constrained;
		CreateConstraint();
	}

	Entity::Component3DPicker PlaneConstraint::GetConstrainedComponent() const
	{
	...
	Optional<Entity::Component3D*> PlaneConstraint::GetBodyComponent() const
	{
	...
	[[maybe_unused]] const bool wasPlaneConstraintRegistered =
		Entity::ComponentRegistry::Register(UniquePtr<Entity::ComponentType<PlaneConstraint>>::Make());
	[[maybe_unused]] const bool wasPlaneConstraintTypeRegistered = Reflection::Registry::RegisterType<PlaneConstraint>();
}
