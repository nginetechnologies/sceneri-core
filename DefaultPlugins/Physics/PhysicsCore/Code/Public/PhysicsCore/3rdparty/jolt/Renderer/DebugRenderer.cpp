...
void DebugRenderer::DrawWireUnitSphereRecursive(
	Mat44Arg inMatrix, ColorArg inColor, Vec3Arg inDir1, Vec3Arg inDir2, Vec3Arg inDir3, int inLevel
)
{
	if (inLevel == 0)
	{
...
void DebugRenderer::Create8thSphereRecursive(
...
void DebugRenderer::Create8thSphere(
	Array<uint32>& ioIndices,
	Array<Vertex>& ioVertices,
	Vec3Arg inDir1,
	Vec3Arg inDir2,
...
void DebugRenderer::Initialize()
{
	...
	auto sphere_support = [](Vec3Arg inDirection)
	{
		return inDirection;
	};
	...
	for (int level = sMaxLevel; level >= 1; --level)
	{
		...
		// Capsule bottom half sphere
		{
			...
			Create8thSphere(
				capsule_bottom_indices,
				capsule_bottom_vertices,
				-Vec3::sAxisX(),
				-Vec3::sAxisY(),
				Vec3::sAxisZ(),
				Float2(0.25f, 0.25f),
...
void DebugRenderer::DrawCapsule(
	Mat44Arg inMatrix, float inHalfHeightOfCylinder, float inRadius, ColorArg inColor, ECastShadow inCastShadow, EDrawMode inDrawMode
)
{
	JPH_PROFILE_FUNCTION();
	...
	AABox local_bounds(
		Vec3(-inRadius, -inHalfHeightOfCylinder - inRadius, -inRadius),
		Vec3(inRadius, inHalfHeightOfCylinder + inRadius, inRadius)
	);
	...
	// Draw middle part
	DrawGeometry(
		inMatrix * Mat44::sScale(Vec3(inRadius, inHalfHeightOfCylinder, inRadius)),
		world_bounds,
		radius_sq,
		inColor,
		mCapsuleMid,
		ECullMode::CullBackFace,
		inCastShadow,
		inDrawMode
	);
}
...
void DebugRenderer::DrawCylinder(
	Mat44Arg inMatrix, float inHalfHeight, float inRadius, ColorArg inColor, ECastShadow inCastShadow, EDrawMode inDrawMode
)
{
	JPH_PROFILE_FUNCTION();
	...
	DrawGeometry(
		transform,
		mCylinder->mBounds.Transformed(transform),
		Square(inRadius),
		inColor,
		mCylinder,
		ECullMode::CullBackFace,
...
void DebugRenderer::DrawOpenCone(
	Vec3Arg inTop,
	Vec3Arg inAxis,
...
void DebugRenderer::DrawSwingLimits(
	Mat44Arg inMatrix,
...
)
{
	...
	if (geometry == nullptr)
	{
		...
		geometry = new Geometry(
			CreateTriangleBatch(vertices_start, num_vertices, indices_start, num_indices),
			sCalculateBounds(vertices_start, num_vertices)
		);
	}
...
)
{
	...
	Mat44 matrix =
		Mat44(Vec4(inRadius * inAxis, 0), Vec4(inRadius * inNormal, 0), Vec4(inRadius * inNormal.Cross(inAxis), 0), Vec4(inCenter, 1)) *
		Mat44::sRotationY(-inMinAngle);
