...
namespace ngine::Rendering::Primitives
{
	struct Plane
	{
		inline static constexpr Rendering::Index VertexCount = 4;
		inline static constexpr Rendering::Index IndexCount = 6;
		...
		static StaticObject Create(const Math::Vector2f half_extent)
		{
			...
			const Array<VertexPosition, 4, Index, Index> vertexPositions{
			...
			const Array<VertexNormals, 4, Index, Index> vertexNormals{
			...
			const Array<VertexTextureCoordinate, 4, Index, Index>
				textureCoordinates{Math::Vector2f{0.0f, 1.0f}, Math::Vector2f{1.0f, 1.0f}, Math::Vector2f{1.0f, 0.0f}, Math::Vector2f{0.0f, 0.0f}};
			staticObject.GetVertexElementView<VertexTextureCoordinate>().CopyFrom(textureCoordinates.GetDynamicView());

			const Array<Rendering::Index, 6, Index, Index> indices{0u, 1u, 2u, 2u, 3u, 0u};
