...
JPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(ConvexHullShapeSettings
    ...
    JPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mPoints) JPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mMaxConvexRadius)
      JPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mMaxErrorConvexRadius) JPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mHullTolerance)}
	...
	, mConvexRadius(inSettings.mMaxConvexRadius)
{
	...
	if (result == ConvexHullBuilder::EResult::Success)
	{
		...
		if (max_error_distance > 4.0f * max(coplanar_distance, inSettings.mHullTolerance)) // Coplanar distance could be bigger than the allowed
		                                                                                   // tolerance if the points are far apart
		{
			outResult.SetError(StringFormat(
				"Hull building failed, point %d had an error of %g (relative to tolerance: %g)",
				max_error_idx,
				(double)max_error_distance,
				double(max_error_distance / inSettings.mHullTolerance)
			));
			return;
		}
	}
	...
	// - How to find the inertia tensor (or other mass properties) of a 3D solid body represented by a triangle mesh (Draft) - Jonathan Blow,
	// Atman J Binstock (http://number-none.com/blow/inertia/bb_inertia.doc)
	Mat44 covariance_canonical(
		Vec4(1.0f / 60.0f, 1.0f / 120.0f, 1.0f / 120.0f, 0),
		Vec4(1.0f / 120.0f, 1.0f / 60.0f, 1.0f / 120.0f, 0),
		Vec4(1.0f / 120.0f, 1.0f / 120.0f, 1.0f / 60.0f, 0),
		Vec4(0, 0, 0, 1)
	);
	Mat44 covariance_matrix = Mat44::sZero();
	for (BuilderFace* f : builder_faces)
	{
		// Fourth point of the tetrahedron is at the center of mass, we subtract it from the other points so we get a tetrahedron with one
		// vertex at zero The first point on the face will be used to form a triangle fan
		Edge* e = f->mFirstEdge;
	...
	if (mConvexRadius > 0.0f)
	{
		for (const Point& point : mPoints)
			if (point.mNumFaces != 1) // If we have a single face, shifting back is easy and we don't need to reduce the convex radius
			{
				...
				if (det_n == 0.0f)
				{
					// If the determinant is zero, the matrix is not invertible so no solution exists to move the point backwards and we have to
					// choose a convex radius of zero
					mConvexRadius = 0.0f;
					break;
				}
	...
	mInnerRadius = max(0.0f, mInnerRadius); // Clamp against zero, this should do nothing as the shape is centered around the center of mass
	                                        // but for flat convex hulls there may be numerical round off issues
...
ConvexHullShape::ConvexHullShape(const ConvexHullShape& other)
	: ConvexShape(other)
	, mCenterOfMass(other.mCenterOfMass)
	, mInertia(other.mInertia)
	, mLocalBounds(other.mLocalBounds)
	, mPoints(other.mPoints)
	, mFaces(other.mFaces)
	, mPlanes(other.mPlanes)
	, mVertexIdx(other.mVertexIdx)
	, mConvexRadius(other.mConvexRadius)
...
MassProperties ConvexHullShape::GetMassProperties(const float density) const
{
	MassProperties p;
...
void ConvexHullShape::GetSupportingFace(Vec3Arg inDirection, Vec3Arg inScale, SupportingFace& outVertices) const
{
	...
	// If we have more than 1/2 the capacity of outVertices worth of vertices, we start skipping vertices (note we can't fill the buffer
	// completely since extra edges will be generated by clipping).
...
void ConvexHullShape::GetSubmergedVolume(
...
) const
{
	...
	PolyhedronSubmergedVolumeCalculator::Point* buffer = (PolyhedronSubmergedVolumeCalculator::Point*)
		JPH_STACK_ALLOC(num_points * sizeof(PolyhedronSubmergedVolumeCalculator::Point));
	PolyhedronSubmergedVolumeCalculator submerged_vol_calc(
		inCenterOfMassTransform * Mat44::sScale(inScale),
		&mPoints[0].mPosition,
		sizeof(Point),
		num_points,
		inSurface,
...
void ConvexHullShape::DrawShrunkShape(DebugRenderer* inRenderer, Mat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	const HullNoConvex* support = mConvexRadius > 0.0f
	                                ? static_cast<const HullNoConvex*>(GetSupportFunction(ESupportMode::ExcludeConvexRadius, buffer, inScale))
	                                : nullptr;
...
) const
{
	...
	// Determine if ray hits the shape
	float min_fraction, max_fraction;
	if (CastRayHelper(inRay, min_fraction, max_fraction) && min_fraction < ioCollector.GetEarlyOutFraction()) // Check if this is closer than
	                                                                                                          // the early out fraction
	{
		// Better hit than the current hit
		RayCastResult hit;
...
void ConvexHullShape::CollidePoint(
...
void ConvexHullShape::GetTrianglesStart(
...
) const
{
	...
	new (&ioContext) CHSGetTrianglesContext(
		Mat44::sRotationTranslation(inRotation, inPositionCOM) * Mat44::sScale(inScale),
		ScaleHelpers::IsInsideOut(inScale)
	);
}

int ConvexHullShape::GetTrianglesNext(
...
Shape::Stats ConvexHullShape::GetStats() const
{
	...
	return Stats(
		sizeof(*this) + mPoints.size() * sizeof(Point) + mFaces.size() * sizeof(Face) + mPlanes.size() * sizeof(Plane) +
