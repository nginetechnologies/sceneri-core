...
{
	Convex,      ///< Used by ConvexShape, all shapes that use the generic convex vs convex collision detection system (box, sphere, capsule,
	             ///< tapered capsule, cylinder, triangle)
...
static constexpr EShapeSubType sAllSubShapeTypes[] = {
	...
	EShapeSubType::User7,
	EShapeSubType::User8};
static constexpr EShapeSubType sConvexSubShapeTypes[] = {
	EShapeSubType::Sphere,
	EShapeSubType::Box,
	EShapeSubType::Triangle,
	EShapeSubType::Capsule,
...
static constexpr EShapeSubType sDecoratorSubShapeTypes[] = {
	EShapeSubType::RotatedTranslated, EShapeSubType::Scaled, EShapeSubType::OffsetCenterOfMass};
...
static constexpr const char* sSubShapeTypeNames[] = {
	"Sphere",
	"Box",
	"Triangle",
	"Capsule",
...
public:
	...
	static inline ShapeFunctions& sGet(EShapeSubType inSubType)
	{
		return sRegistry[(int)inSubType];
	}
private:
...
public:
	...
	Shape(EShapeType inType, EShapeSubType inSubType, const ShapeSettings& inSettings, [[maybe_unused]] ShapeResult& outResult)
	...
	Shape(const Shape& other)
		...
		, mShapeSubType(other.mShapeSubType)
	{
	}
	...
	inline EShapeSubType GetSubType() const
	{
		return mShapeSubType;
	}
	...
	/// All shapes are centered around their center of mass. This function returns the center of mass position that needs to be applied to
	/// transform the shape to where it was created.
	...
	/// Get the max number of sub shape ID bits that are needed to be able to address any leaf shape in this shape. Used mainly for checking
	/// that it is smaller or equal than SubShapeID::MaxBits.
	virtual uint GetSubShapeIDBitsRecursive() const = 0;

	/// Get world space bounds including convex radius.
	/// This shape is scaled by inScale in local space first.
	/// This function can be overridden to return a closer fitting world space bounding box, by default it will just transform what
	/// GetLocalBounds() returns.
	...
	/// Returns the radius of the biggest sphere that fits entirely in the shape. In case this shape consists of multiple sub shapes, it
	/// returns the smallest sphere of the parts. This can be used as a measure of how far the shape can be moved without risking going
	/// through geometry.
	virtual float GetInnerRadius() const = 0;

	/// Calculate the mass and inertia of this shape
	virtual MassProperties GetMassProperties(const float density) const = 0;
	/// Calculate the mass and inertia of this shape
	...
	virtual void SetMaterial(const SubShapeID& inSubShapeID, const PhysicsMaterial* pMaterial) = 0;
	...
	/// Note: When you have a CollideShapeResult or ShapeCastResult you should use -mPenetrationAxis.Normalized() as contact normal as
	...
	virtual TransformedShape GetSubShapeTransformedShape(
		const SubShapeID& inSubShapeID, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale, SubShapeID& outRemainder
	) const;
	...
	/// Draw the results of the GetSupportFunction with the convex radius added back on to show any errors introduced by this process (only
	/// relevant for convex shapes)
	...
	{ /* Only implemented for convex shapes */
	}
	...
	/// Cast a ray against this shape, returns true if it finds a hit closer than ioHit.mFraction and updates that fraction. Otherwise ioHit
	/// is left untouched and the function returns false. Note that the ray should be relative to the center of mass of this shape (i.e.
	/// subtract Shape::GetCenterOfMass() from RayCast::mOrigin if you want to cast against the shape in the space it was created). Convex
	/// objects will be treated as solid (meaning if the ray starts inside, you'll get a hit fraction of 0) and back face hits against
	/// triangles are returned. If you want the surface normal of the hit use GetSurfaceNormal(ioHit.mSubShapeID2,
	...
	/// Cast a ray against this shape. Allows returning multiple hits through ioCollector. Note that this version is more flexible but also
	/// slightly slower than the CastRay function that returns only a single hit. If you want the surface normal of the hit use
	...
	/// Note that inPoint should be relative to the center of mass of this shape (i.e. subtract Shape::GetCenterOfMass() from inPoint if you
	/// want to test against the shape in the space it was created). For a mesh shape, this test will only provide sensible information if the
	/// mesh is a closed manifold. For each shape that collides, ioCollector will receive a hit.
	...
	/// Note that not all shapes support all transforms (especially true for scaling), the resulting shape will try to match the transform as
	...
	/// Scale this shape. Note that not all shapes support all scales, this will return a shape that matches the scale as accurately as
	/// possible.
	/// @param inScale The scale to use for this shape (note: this scale is applied to the entire shape in the space it was created, most
	/// function apply the scale in the space of the leaf shapes and from the center of mass!)
	...
	struct alignas(16) GetTrianglesContext
	{
		uint8 mData[4288];
	};
	...
	/// The function returns the amount of triangles that it found (which will be <= inMaxTrianglesRequested), or 0 if there are no more
	/// triangles. Note that the function can return a value < inMaxTrianglesRequested and still have more triangles to process (triangles can
	/// be returned in blocks). Note that the function may return triangles outside of the requested box, only coarse culling is performed on
	...
	///@name Binary serialization of the shape. Note that this saves the 'cooked' shape in a format which will not be backwards compatible for
	/// newer library versions.
	/// In this case you need to recreate the shape from the ShapeSettings object and save it again. The user is expected to call
	/// SaveBinaryState followed by SaveMaterialState and SaveSubShapeState. The stream should be stored as is and the material and shape list
	/// should be saved using the applications own serialization system (e.g. by assigning an ID to each pointer). When restoring data, call
	/// sRestoreFromBinararyState to get the shape and then call RestoreMaterialState and RestoreSubShapeState to restore the pointers to the
	/// external objects.
	///@{
	...
	/// Restore the material references after calling sRestoreFromBinaryState. Note that the exact same materials need to be provided in the
	/// same order as returned by SaveMaterialState.
	virtual void RestoreMaterialState(const PhysicsMaterialRefC* inMaterials, uint inNumMaterials)
	{
		JPH_ASSERT(inNumMaterials == 0);
	}
	...
	/// Restore the shape references after calling sRestoreFromBinaryState. Note that the exact same shapes need to be provided in the same
	/// order as returned by SaveSubShapeState.
	virtual void RestoreSubShapeState(const ShapeRefC* inSubShapes, uint inNumShapes)
	{
		JPH_ASSERT(inNumShapes == 0);
	}
	...
	/// Save this shape, all its children and its materials. Pass in an empty map in ioShapeMap / ioMaterialMap or reuse the same map while
	/// saving multiple shapes to the same stream in order to avoid writing duplicates.
	void SaveWithChildren(StreamOut& inStream, ShapeToIDMap& ioShapeMap, MaterialToIDMap& ioMaterialMap) const;

	/// Restore a shape, all its children and materials. Pass in an empty map in ioShapeMap / ioMaterialMap or reuse the same map while
	/// reading multiple shapes from the same stream in order to restore duplicates.
	...
	/// Get stats of this shape. Use for logging / data collection purposes only. Does not add values from child shapes, use GetStatsRecursive
	/// for this.
	...
	///< Volume of this shape (m^3). Note that for compound shapes the volume may be incorrect since child shapes can overlap which is not
	///< accounted for.
