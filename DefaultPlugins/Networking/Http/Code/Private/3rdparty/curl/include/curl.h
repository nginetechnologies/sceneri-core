...
/* HP-UX systems version 9, 10 and 11 lack sys/select.h and so does oldish
   ...
   require it! */
	...
	defined(__INTEGRITY) || defined(ANDROID) || defined(__ANDROID__) || defined(__OpenBSD__) || defined(__CYGWIN__) || defined(AMIGA) || \
...
#if PLATFORM_WINDOWS
#define CURL_CALLING_CONV __stdcall
#else
#define CURL_CALLING_CONV
#endif
	 ...
	 * target handle.
	 */
	CURL_EXTERN curl_mime* CURL_CALLING_CONV curl_mime_init(CURL* easy);
	 ...
	 */
	CURL_EXTERN void CURL_CALLING_CONV curl_mime_free(curl_mime* mime);
	 ...
	 * the created part.
	 */
	CURL_EXTERN curl_mimepart* CURL_CALLING_CONV curl_mime_addpart(curl_mime* mime);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_name(curl_mimepart* part, const char* name);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_filename(curl_mimepart* part, const char* filename);
	 ...
	 * Set mime part type.
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_type(curl_mimepart* part, const char* mimetype);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_data(curl_mimepart* part, const char* data, size_t datasize);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_filedata(curl_mimepart* part, const char* filename);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_data_cb(
		curl_mimepart* part,
		curl_off_t datasize,
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_subparts(curl_mimepart* part, curl_mime* subparts);
	/*
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_mime_headers(curl_mimepart* part, struct curl_slist* headers, int take_ownership);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_global_init(long flags);
	 ...
	 */
	CURL_EXTERN CURLcode CURL_CALLING_CONV curl_global_init_mem(
		long flags, curl_malloc_callback m, curl_free_callback f, curl_realloc_callback r, curl_strdup_callback s, curl_calloc_callback c
	);
	 ...
	 * that uses libcurl
	 */
	CURL_EXTERN void CURL_CALLING_CONV curl_global_cleanup(void);
	...
	CURL_EXTERN CURLsslset CURL_CALLING_CONV curl_global_sslset(curl_sslbackend id, const char* name, const curl_ssl_backend*** avail);
	 ...
	 */
	CURL_EXTERN struct curl_slist* CURL_CALLING_CONV curl_slist_append(struct curl_slist*, const char*);
	 ...
	 */
	CURL_EXTERN void CURL_CALLING_CONV curl_slist_free_all(struct curl_slist*);
...
#if defined(__GNUC__) && defined(__GNUC_MINOR__) && ((__GNUC__ > 4) || (__GNUC__ == 4 && __GNUC_MINOR__ >= 3)) && !defined(__cplusplus) && \
