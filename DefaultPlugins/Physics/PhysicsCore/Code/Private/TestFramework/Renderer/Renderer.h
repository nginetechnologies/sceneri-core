...
#include <TestFramework/Image/Surface.h>
#include <TestFramework/Renderer/Frustum.h>
#include <TestFramework/Renderer/ConstantBuffer.h>
#include <TestFramework/Renderer/PipelineState.h>
#include <TestFramework/Renderer/CommandQueue.h>
#include <TestFramework/Renderer/DescriptorHeap.h>
...
namespace JPH::DebugRendering
{
	...
	public:
		...
		ComPtr<ID3DBlob> CreateVertexShader(const ngine::IO::ConstZeroTerminatedPathView fileName);

		/// Compile a pixel shader
		ComPtr<ID3DBlob> CreatePixelShader(const ngine::IO::ConstZeroTerminatedPathView fileName);
		...
		unique_ptr<PipelineState> CreatePipelineState(
		...
		/// Recycle a buffer on the upload heap. This puts it back in a cache and will reuse it when it is certain the GPU is no longer
		/// referencing it.
	...
	private:
		...
		ComPtr<ID3D12Resource> mRenderTargets[cFrameCount]; ///< Two render targets (we're double buffering in order for the CPU to continue
		                                                    ///< while the GPU is rendering)
		...
		ComPtr<ID3D12CommandQueue> mCommandQueue; ///< The command queue that will execute commands (there's only 1 since we want to finish
		                                          ///< rendering 1 frame before moving onto the next)
		ComPtr<ID3D12GraphicsCommandList> mCommandList; ///< The command list
		ComPtr<ID3D12RootSignature>
			mRootSignature; ///< The root signature, we have a simple application so we only need 1, which is suitable for all our shaders
		...
		ResourceCache mDelayCached[cFrameCount]; ///< List of reusable ID3D12Resources that are potentially referenced by the GPU so can be used
		                                         ///< only when the GPU finishes
		Array<ComPtr<ID3D12Object>>
			mDelayReleased[cFrameCount]; ///< Objects that are potentially referenced by the GPU so can only be freed when the GPU finishes
