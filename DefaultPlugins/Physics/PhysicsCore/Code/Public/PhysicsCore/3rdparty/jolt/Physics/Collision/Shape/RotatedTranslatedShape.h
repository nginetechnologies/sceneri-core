...
public:
	...
	{
		mRotation = rotation;
		mIsRotationIdentity = rotation.IsClose(Quat::sIdentity());
	}
	...
	{
		mCenterOfMass = position + mRotation * mInnerShape->GetCenterOfMass();
	}

	void							SetPositionAndRotation(const Vec3 position, const Quat rotation)
	{
		mCenterOfMass = position + rotation * mInnerShape->GetCenterOfMass();
		mRotation = rotation;
		mIsRotationIdentity = rotation.IsClose(Quat::sIdentity());
	}
