...
namespace JPH::DebugRendering
{
	...
	{
		return mConvertRGBToAlpha == inRHS.mConvertRGBToAlpha && mConvertAlphaToRGB == inRHS.mConvertAlphaToRGB &&
		       mConvertToGrayScale == inRHS.mConvertToGrayScale && mInvertAlpha == inRHS.mInvertAlpha &&
		       mColorKeyAlpha == inRHS.mColorKeyAlpha && mColorKeyStart == inRHS.mColorKeyStart && mColorKeyEnd == inRHS.mColorKeyEnd &&
...
#define COL(s) (CMP(s, 0) + CMP(s, 1) + CMP(s, 2) + CMP(s, 3))
	...
	static void sComputeTranslationTable(
		const FormatDescription& inSrcDesc, const FormatDescription& inDstDesc, uint32* outMask, uint32* outShift, uint32* outMap
	)
	{
		...
		for (int c = 0; c < 4; ++c)
		{
			...
			float scale = float(dst_shifted_mask) / (float)src_shifted_mask;

			uint32 entry = 0;

			if (src_shifted_mask != 0)
				for (; entry <= src_shifted_mask; ++entry)
					outMap[256 * c + entry] = uint32(round(scale * (float)entry)) << dst_shift;
