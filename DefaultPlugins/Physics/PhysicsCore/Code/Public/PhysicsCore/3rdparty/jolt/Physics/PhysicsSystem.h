...
///
/// The main simulation is performed by the Update() call on multiple threads (if the JobSystem is configured to use them). Please refer to
/// the general architecture overview in the Docs folder for more information.
...
public:
	...
	/// @param inBroadPhaseLayerInterface Information on the mapping of object layers to broad phase layers, note since this is a virtual
	/// interface, the instance needs to stay alive during the lifetime of the PhysicsSystem
	/// @param inObjectVsBroadPhaseLayerFilter Filter callback function that is used to determine if an object layer collides with a broad
	/// phase layer.
	...
	{
		mBodyManager.SetBodyActivationListener(inListener);
	}
	BodyActivationListener* GetBodyActivationListener() const
	{
		return mBodyManager.GetBodyActivationListener();
	}
	...
	{
		mContactManager.SetContactListener(inListener);
	}
	ContactListener* GetContactListener() const
	{
		return mContactManager.GetContactListener();
	}
	...
	{
		mContactManager.SetCombineFriction(inCombineFriction);
	}
	...
	{
		mConstraintManager.Add(inConstraints, inNumber);
	}
	...
	{
		mConstraintManager.Remove(inConstraints, inNumber);
	}

	/// Get a list of all constraints
	Constraints GetConstraints() const
	{
		return mConstraintManager.GetConstraints();
	}
	...
	// Drawing properties
	static bool sDrawMotionQualityLinearCast; ///< Draw debug info for objects that perform continuous collision detection through the linear
	                                          ///< cast motion quality
	...
	{
		mBodyManager.Draw(inSettings, mPhysicsSettings, inRenderer, inBodyFilter);
	}

	/// Draw the constraints only (debugging purposes)
	void DrawConstraints(DebugRenderer* inRenderer)
	{
		mConstraintManager.DrawConstraints(inRenderer);
	}

	/// Draw the constraint limits only (debugging purposes)
	void DrawConstraintLimits(DebugRenderer* inRenderer)
	{
		mConstraintManager.DrawConstraintLimits(inRenderer);
	}

	/// Draw the constraint reference frames only (debugging purposes)
	void DrawConstraintReferenceFrame(DebugRenderer* inRenderer)
	{
		mConstraintManager.DrawConstraintReferenceFrame(inRenderer);
	}
#endif // JPH_DEBUG_RENDERER

	/// Set gravity value, note that this is only used for bodies to query the gravity that is affecting them
	/// It does not affect any forces in the world
	void SetGlobalGravity(Vec3Arg inGravity)
	{
		mGlobalGravity = inGravity;
	}
	...
	{
		return mBodyLockInterfaceNoLock;
	}
	...
	{
		return mBodyLockInterfaceLocking;
	}

	/// Get an broadphase layer filter that uses the default pair filter and a specified object layer to determine if broadphase layers
	/// collide
	DefaultBroadPhaseLayerFilter GetDefaultBroadPhaseLayerFilter(ObjectLayer inLayer) const
	{
		return DefaultBroadPhaseLayerFilter(mObjectVsBroadPhaseLayerFilter, inLayer);
	}
	...
	{
		return DefaultObjectLayerFilter(mObjectLayerPairFilter, inLayer);
	}
	...
	{
		return mBodyManager.GetNumActiveBodies();
	}
	...
	{
		return mBodyManager.GetBodyStats();
	}
	...
	{
		return mBodyManager.GetBodyIDs(outBodyIDs);
	}
	...
	{
		return mBodyManager.GetActiveBodies(outBodyIDs);
	}

#ifdef JPH_TRACK_BROADPHASE_STATS
	/// Trace the accumulated broadphase stats to the TTY
	void ReportBroadphaseStats()
	{
		mBroadPhase->ReportStats();
	}
...
private:
	...
	Vec3 mGlobalGravity = Vec3(0, 0, 0);
