...
MassProperties CompoundShape::GetMassProperties([[maybe_unused]] const float density) const
{
	MassProperties p;
...
void CompoundShape::SetMaterial(const SubShapeID& inSubShapeID, const PhysicsMaterial* pMaterial)
{
	// Decode sub shape index
	SubShapeID remainder;
	uint32 index = GetSubShapeIndexFromID(inSubShapeID, remainder);
	...
	// Pass call on
	const_cast<Shape*>(mSubShapes[index].mShape.GetPtr())->SetMaterial(remainder, pMaterial);
}
