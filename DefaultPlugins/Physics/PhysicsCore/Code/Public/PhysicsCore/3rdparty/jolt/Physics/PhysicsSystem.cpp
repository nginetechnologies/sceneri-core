...
)
{
	...
	mBodyManager.Init(inNumBodyMutexes, inBroadPhaseLayerInterface);
...
void PhysicsSystem::Update(
	float inDeltaTime, int inCollisionSteps, int inIntegrationSubSteps, TempAllocator* inTempAllocator, JobSystem* inJobSystem
)
{
	...
	float warm_start_impulse_ratio = mPhysicsSettings.mConstraintWarmStart && mPreviousSubStepDeltaTime > 0.0f
	                                   ? sub_step_delta_time / mPreviousSubStepDeltaTime
	                                   : 0.0f;
	...
	int num_step_listener_jobs = mStepListeners.empty() ? 0
	                                                    : max(
																													1,
																													min(
																														(int)mStepListeners.size() / mPhysicsSettings.mStepListenersBatchSize /
	...
	int num_determine_active_constraints_jobs = max(
		1,
		min(
			((int)mConstraintManager.GetNumConstraints() + cDetermineActiveConstraintsBatchSize - 1) / cDetermineActiveConstraintsBatchSize,
			max_concurrency - 2
		)
	);

	// Number of find collisions jobs to run depends on number of active bodies.
	// Note that when we have more than 1 thread, we always spawn at least 2 find collisions jobs so that the first job can wait for build
	// islands from constraints (which may activate additional bodies that need to be processed) while the second job can start processing
	// collision work.
	int num_find_collisions_jobs =
		max(max_concurrency == 1 ? 1 : 2, min(((int)num_active_bodies + cActiveBodiesBatchSize - 1) / cActiveBodiesBatchSize, max_concurrency));

	// Number of integrate velocity jobs depends on number of active bodies.
	int num_integrate_velocity_jobs =
		max(1, min(((int)num_active_bodies + cIntegrateVelocityBatchSize - 1) / cIntegrateVelocityBatchSize, max_concurrency));

	{
		...
		for (int step_idx = 0; step_idx < inCollisionSteps; ++step_idx)
		{
			...
			// This job must finish before integrating velocities. Until then the positions will not be updated neither will bodies be added /
			// removed.
			step.mUpdateBroadphaseFinalize = inJobSystem->CreateJob(
				"UpdateBroadPhaseFinalize",
				cColorUpdateBroadPhaseFinalize,
				[&context, &step]()
				{
				...
				num_find_collisions_jobs + 2
			); // depends on: find collisions, broadphase prepare update, finish building jobs
			...
			step.mBroadPhasePrepare = inJobSystem->CreateJob(
				"UpdateBroadPhasePrepare",
				cColorUpdateBroadPhasePrepare,
				[&context, &step]()
				{
			...
			step.mActiveFindCollisionJobs = ~PhysicsUpdateContext::JobMask(0) >>
			                                (sizeof(PhysicsUpdateContext::JobMask) * 8 - num_find_collisions_jobs);
			step.mFindCollisions.resize(num_find_collisions_jobs);
			for (int i = 0; i < num_find_collisions_jobs; ++i)
			{
				// Build islands from constraints may activate additional bodies, so the first job will wait for this to finish in order to not miss
				// any active bodies
				int num_dep_build_islands_from_constraints = i == 0 ? 1 : 0;
				step.mFindCollisions[i] = inJobSystem->CreateJob(
					"FindCollisions",
					cColorFindCollisions,
					[&step, i]()
					{
						step.mContext->mPhysicsSystem->JobFindCollisions(&step, i);
					},
					num_determine_active_constraints_jobs + 1 + num_dep_build_islands_from_constraints
				); // depends on: apply gravity, determine active constraints, finish building jobs, build islands from constraints
			}
			...
			if (is_first_step)
			{
				...
				context.mActiveConstraints =
					static_cast<Constraint**>(inTempAllocator->Allocate(mConstraintManager.GetNumConstraints() * sizeof(Constraint*)));
			...
			step.mSetupVelocityConstraints = inJobSystem->CreateJob(
				"SetupVelocityConstraints",
				cColorSetupVelocityConstraints,
				[&context, &step]()
				{
			...
			step.mBuildIslandsFromConstraints = inJobSystem->CreateJob(
				"BuildIslandsFromConstraints",
				cColorBuildIslandsFromConstraints,
				[&context, &step]()
				{
					context.mPhysicsSystem->JobBuildIslandsFromConstraints(&context, &step);

					step.mFindCollisions[0].RemoveDependency(
					); // The first collisions job cannot start running until we've finished building islands and activated all bodies
			...
			for (int i = 0; i < num_determine_active_constraints_jobs; ++i)
				step.mDetermineActiveConstraints[i] = inJobSystem->CreateJob(
					"DetermineActiveConstraints",
					cColorDetermineActiveConstraints,
					[&context, &step]()
					{
					...
					num_step_listener_jobs > 0 ? num_step_listener_jobs : previous_step_dependency_count
				); // depends on: step listeners (or previous step if no step listeners)

			// This job calls the step listeners
			step.mStepListeners.resize(num_step_listener_jobs);
			for (int i = 0; i < num_step_listener_jobs; ++i)
				step.mStepListeners[i] = inJobSystem->CreateJob(
					"StepListeners",
					cColorStepListeners,
					[&context, &step]()
					{
			...
			step.mFinalizeIslands = inJobSystem->CreateJob(
				"FinalizeIslands",
				cColorFinalizeIslands,
				[&context, &step]()
				{
					// Validate that all find collision jobs have stopped
					JPH_ASSERT(step.mActiveFindCollisionJobs == 0);
				...
				num_find_collisions_jobs + 2
			); // depends on: find collisions, build islands from constraints, finish building jobs

			// Unblock previous job
			// Note: technically we could release find collisions here but we don't want to because that could make them run before 'setup
			// velocity constraints' which means that job won't have a thread left
			step.mBuildIslandsFromConstraints.RemoveDependency();

			// This job will call the contact removed callbacks
			step.mContactRemovedCallbacks = inJobSystem->CreateJob(
				"ContactRemovedCallbacks",
				cColorContactRemovedCallbacks,
				[&context, &step]()
				{
				...
				1
			); // depends on the find ccd contacts of the last sub step
			...
			step.mBodySetIslandIndex = inJobSystem->CreateJob(
				"BodySetIslandIndex",
				cColorBodySetIslandIndex,
				[&context, &step]()
				{
			...
			if (!is_last_step)
			{
				...
				step.mStartNextStep = inJobSystem->CreateJob(
					...
					[this, next_step]()
					{
						...
						{
							// Kick the active constraints jobs immediately
					...
					max_concurrency + 3
				); // depends on: solve position constraints of the last step, body set island index, contact removed callbacks, finish building the
				   // previous step
			}
			...
			for (int sub_step_idx = 0; sub_step_idx < inIntegrationSubSteps; ++sub_step_idx)
			{
				...
				int num_dependencies_solve_velocity_constraints =
					is_first_sub_step ? 3 : 2; // in first sub step depends on: finalize islands, setup velocity constraints, in later sub steps
				                             // depends on: previous sub step finished. For both: finish building jobs.
				sub_step.mSolveVelocityConstraints.resize(max_concurrency);
				for (int i = 0; i < max_concurrency; ++i)
					sub_step.mSolveVelocityConstraints[i] = inJobSystem->CreateJob(
						"SolveVelocityConstraints",
						cColorSolveVelocityConstraints,
						[&context, &sub_step]()
						{
				...
				int num_dependencies_integrate_velocity =
					is_first_sub_step ? 2 + max_concurrency
														: 1 + max_concurrency; // depends on: broadphase update finalize in first step, solve velocity constraints in
				                                           // all steps. For both: finish building jobs.
				sub_step.mPreIntegrateVelocity = inJobSystem->CreateJob(
					"PreIntegrateVelocity",
					cColorPreIntegrateVelocity,
					[&context, &sub_step]()
					{
				...
				for (int i = 0; i < num_integrate_velocity_jobs; ++i)
					sub_step.mIntegrateVelocity[i] = inJobSystem->CreateJob(
						"IntegrateVelocity",
						cColorIntegrateVelocity,
						[&context, &sub_step]()
						{
						...
						2
					); // depends on: pre integrate velocity, finish building jobs.

				// Unblock previous job
				sub_step.mPreIntegrateVelocity.RemoveDependency();

				// This job will finish the position update of all active bodies
				sub_step.mPostIntegrateVelocity = inJobSystem->CreateJob(
					"PostIntegrateVelocity",
					cColorPostIntegrateVelocity,
					[&context, &sub_step]()
					{
					...
					num_integrate_velocity_jobs + 1
				); // depends on: integrate velocity, finish building jobs

				// Unblock previous jobs
				JobHandle::sRemoveDependencies(sub_step.mIntegrateVelocity);

				// This job will update the positions and velocities for all bodies that need continuous collision detection
				sub_step.mResolveCCDContacts = inJobSystem->CreateJob(
					"ResolveCCDContacts",
					cColorResolveCCDContacts,
					[&context, &sub_step]()
					{
					...
					2
				); // depends on: integrate velocities, detect ccd contacts (added dynamically), finish building jobs.
				...
				for (int i = 0; i < max_concurrency; ++i)
					sub_step.mSolvePositionConstraints[i] = inJobSystem->CreateJob(
						"SolvePositionConstraints",
						cColorSolvePositionConstraints,
						[&context, &sub_step]()
						{
						...
						2
					); // depends on: resolve ccd contacts, finish building jobs.
				...
				if (!is_last_sub_step)
				{
					...
					sub_step.mStartNextSubStep = inJobSystem->CreateJob(
						"StartNextSubStep",
						cColorStartNextSubStep,
						[&next_sub_step]()
						{
						...
						max_concurrency + 1
					); // depends on: solve position constraints, finish building jobs.
				}
				else
...
void PhysicsSystem::JobStepListeners(PhysicsUpdateContext::Step* ioStep)
{
	...
	// Can activate bodies only (we cache the amount of active bodies at the beginning of the step in mNumActiveBodiesAtStepStart so we cannot
	// deactivate here)
...
void PhysicsSystem::JobDetermineActiveConstraints(PhysicsUpdateContext::Step* ioStep) const
{
	...
	for (;;)
	{
		...
		// leading to an inconsistent shapshot)
		...
		if (num_active_constraints > 0)
		{
			...
			memcpy(
				ioStep->mContext->mActiveConstraints + active_constraint_idx,
				active_constraints,
				num_active_constraints * sizeof(Constraint*)
			);
		}
	}
}
...
void PhysicsSystem::TrySpawnJobFindCollisions(PhysicsUpdateContext::Step* ioStep) const
{
	...
	uint desired_num_jobs = min(
		(num_body_pairs + cNarrowPhaseBatchSize - 1) / cNarrowPhaseBatchSize +
			(num_active_bodies + cActiveBodiesBatchSize - 1) / cActiveBodiesBatchSize,
		max_jobs
	);
		...
		for (uint job_index = 0; job_index < max_jobs; ++job_index)
		{
			...
			if ((current_active_jobs & job_mask) == 0)
			{
				...
				if ((prev_value & job_mask) == 0)
				{
					...
					// Start the job
					JobHandle job = ioStep->mContext->mJobSystem->CreateJob(
						"FindCollisions",
						cColorFindCollisions,
						[step = ioStep, job_index]()
						{
	...
	for (;;)
	{
		...
		if (active_bodies_read_idx < num_active_bodies)
		{
			...
			if (ioStep->mActiveBodyReadIdx.compare_exchange_strong(active_bodies_read_idx, active_bodies_read_idx_end))
			{
				...
				public:
					...
					virtual void AddHit(const BodyPair& inPair) override
					{
						...
						else
						{
							...
							mStep->mContext->mBodyPairs[mJobIndex * mStep->mMaxBodyPairsPerQueue + queue.mWriteIdx % mStep->mMaxBodyPairsPerQueue] =
								inPair;
							++queue.mWriteIdx;
						}
					}
				...
				mBroadPhase->FindCollidingPairs(
					active_bodies,
					batch_size,
...
void PhysicsSystem::ProcessBodyPair(ContactAllocator& ioContactAllocator, const BodyPair& inBodyPair)
{
	...
	JPH_DET_LOG(
		"ProcessBodyPair: id1: " << inBodyPair.mBodyA << " id2: " << inBodyPair.mBodyB << " p1: " << body1->GetCenterOfMassPosition() << " p2: "
														 << body2->GetCenterOfMassPosition() << " r1: " << body1->GetRotation() << " r2: " << body2->GetRotation()
	);

	// Ensure that body1 is dynamic, this ensures that we do the collision detection in the space of a moving body, which avoids accuracy
	// problems when testing a very large static object against a small dynamic object Ensure that body1 id < body2 id for dynamic vs dynamic
	// Keep body order unchanged when colliding with a sensor
	if ((!body1->IsDynamic() || (body2->IsDynamic() && inBodyPair.mBodyB < inBodyPair.mBodyA)) && !body2->IsSensor())
		swap(body1, body2);
	...
	if (!pair_handled)
	{
		...
		settings.mActiveEdgeMode = mPhysicsSettings.mCheckActiveEdges ? EActiveEdgeMode::CollideOnlyWithActive
				...
				virtual void AddHit(const CollideShapeResult& inResult) override
				{
					...
					if (manifold == mManifolds.end())
					{
						...
						if (mManifolds.size() == mManifolds.capacity())
						{
							...
							*manifold = {
								{world_space_normal, inResult.mPenetrationDepth, inResult.mSubShapeID1, inResult.mSubShapeID2, {}, {}},
								world_space_normal};
						}
						else
						{
							// Not full, create new manifold
							mManifolds.push_back(
								{{world_space_normal, inResult.mPenetrationDepth, inResult.mSubShapeID1, inResult.mSubShapeID2, {}, {}}, world_space_normal}
							);
					...
					ManifoldBetweenTwoFaces(
						inResult.mContactPointOn1,
						inResult.mContactPointOn2,
						inResult.mPenetrationAxis,
						Square(settings.mSpeculativeContactDistance) + settings.mManifoldToleranceSq,
						inResult.mShape1Face,
						inResult.mShape2Face,
					...
					if (manifold->mWorldSpaceContactPointsOn1.size() > 32)
						PruneContactPoints(
							mBody1->GetCenterOfMassPosition(),
							manifold->mFirstWorldSpaceNormal,
							manifold->mWorldSpaceContactPointsOn1,
							manifold->mWorldSpaceContactPointsOn2
						);
				}
		...
		else
		{
			...
			{
			public:
				NonReductionCollideShapeCollector(
					PhysicsSystem* inSystem,
					ContactAllocator& ioContactAllocator,
					Body* inBody1,
					Body* inBody2,
					const ContactConstraintManager::BodyPairHandle& inPairHandle
				)
					: mSystem(inSystem)
					, mContactAllocator(ioContactAllocator)
					, mBody1(inBody1)
...
void PhysicsSystem::JobFinalizeIslands(PhysicsUpdateContext* ioContext)
{
	...
	mIslandBuilder.Finalize(
		mBodyManager.GetActiveBodiesUnsafe(),
		mBodyManager.GetNumActiveBodies(),
		mContactManager.GetNumConstraints(),
		ioContext->mTempAllocator
	);
}
...
void PhysicsSystem::JobIntegrateVelocity(const PhysicsUpdateContext* ioContext, PhysicsUpdateContext::SubStep* ioSubStep)
{
	...
	for (;;)
	{
		...
		while (active_body_idx < active_body_idx_end)
		{
			...
			// angular velocity and a limited reduction in linear velocity. When simulation the rotation first before doing the translation, the
			// body will be able to rotate away from the contact point allowing the center of mass to approach the surface. When using approach
			// (2) in this case what will happen is that we will immediately detect the same collision again (the body has not rotated and the
			// body was already colliding at the end of the previous time step) resulting in a lot of stolen time and the body appearing to be
			// frozen in an unnatural pose (like it is glued at an angle to the surface). (2) obviously has some negative side effects too as
			// simulating the rotation first may cause it to tunnel through a small object that the linear cast might have otherwise dectected. In
			// any case a linear cast is not good for detecting tunneling due to angular rotation, so we don't care about that too much (you'd
			// need a full cast to take angular effects into account).
			...
			switch (mp->GetMotionQuality())
			{
				...
				case EMotionQuality::LinearCast:
					if (body.IsDynamic())
					{
						...
						JPH_ASSERT(
							inner_radius > 0.0f,
							"The shape has no inner radius, this makes the shape unsuitable for the linear cast motion quality as we cannot move it "
							"without risking tunneling."
						);
						...
						if (delta_pos.LengthSq() > linear_cast_threshold_sq)
						{
							...
							new (&ioSubStep->mCCDBodies[ccd_body_idx]) CCDBody(
								body_id,
								delta_pos,
								linear_cast_threshold_sq,
								min(mPhysicsSettings.mPenetrationSlop, mPhysicsSettings.mLinearCastMaxPenetration * inner_radius)
							);
...
void PhysicsSystem::JobPostIntegrateVelocity(PhysicsUpdateContext* ioContext, PhysicsUpdateContext::SubStep* ioSubStep) const
{
	...
	else
	{
		...
		int num_continuous_collision_jobs =
			min(int(ioSubStep->mNumCCDBodies + cNumCCDBodiesPerJob - 1) / cNumCCDBodiesPerJob, ioContext->GetMaxConcurrency());
		...
		{
			JobHandle job = ioContext->mJobSystem->CreateJob(
				"FindCCDContacts",
				cColorFindCCDContacts,
				[ioContext, ioSubStep]()
				{
	...
	for (;;)
	{
		...
		if (sDrawMotionQualityLinearCast)
		{
			...
			body.GetShape()->Draw(
				...
				Mat44::sTranslation(ccd_body.mDeltaPosition) * com,
				Vec3::sReplicate(1.0f),
				Color::sRed,
				false,
				true
		...
		public:
			CCDNarrowPhaseCollector(
				...
				ContactConstraintManager& inContactConstraintManager,
				CCDBody& inCCDBody,
				ShapeCastResult& inResult,
				float inDeltaTime
			)
			...
			virtual void AddHit(const ShapeCastResult& inResult) override
			{
				...
				if (fraction < mCCDBody.mFractionPlusSlop)
				{
					...
					// Let the extra distance that we can travel along delta_pos be 'dist': mMaxPenetration / dist = cos(angle between normal and
					// delta_pos) = normal . delta_pos / |delta_pos|
					...
					if (denominator > mCCDBody.mMaxPenetration) // Avoid dividing by zero, if extra hit fraction > 1 there's also no point in
					                                            // continuing
					{
						...
						if (fraction_plus_slop < mCCDBody.mFractionPlusSlop)
						{
							...
							if (mValidateBodyPair)
							{
								...
								ValidateResult validate_result =
									mContactConstraintManager.ValidateContactPoint(mBodyManager.GetBody(mCCDBody.mBodyID1), body2, inResult);
		...
		{
		public:
			CCDBroadPhaseCollector(
				const CCDBody& inCCDBody,
				const Body& inBody1,
				const ShapeCast& inShapeCast,
				ShapeCastSettings& inShapeCastSettings,
				CCDNarrowPhaseCollector& ioCollector,
				const BodyManager& inBodyManager,
				PhysicsUpdateContext::SubStep* inSubStep,
				float inDeltaTime
			)
				: mCCDBody(inCCDBody)
				, mBody1(inBody1)
			...
			virtual void AddHit(const BroadPhaseCastResult& inResult) override
			{
				...
				if (hit_fraction > max(FLT_MIN, GetEarlyOutFraction())) // If early out fraction <= 0, we have the possibility of finding a deeper
				                                                        // hit so we need to clamp the early out fraction
					return;
				...
				ShapeCast
					relative_cast(mShapeCast.mShape, mShapeCast.mScale, mShapeCast.mCenterOfMassStart, direction, mShapeCast.mShapeWorldBounds);
		...
		// Check if we collide with any other body. Note that we use the non-locking interface as we know the broadphase cannot be modified at
		// this point.
		ShapeCast shape_cast(body.GetShape(), Vec3::sReplicate(1.0f), body.GetCenterOfMassTransform(), ccd_body.mDeltaPosition);
		CCDBroadPhaseCollector
			bp_collector(ccd_body, body, shape_cast, settings, np_collector, mBodyManager, ioSubStep, ioContext->mSubStepDeltaTime);
		mBroadPhase
			->CastAABoxNoLock({shape_cast.mShapeWorldBounds, shape_cast.mDirection}, bp_collector, broadphase_layer_filter, object_layer_filter);
		...
		if (ccd_body.mFractionPlusSlop < 1.0f)
		{
			...
			ContactManifold manifold;
			ManifoldBetweenTwoFaces(
				cast_shape_result.mContactPointOn1,
				cast_shape_result.mContactPointOn2,
				cast_shape_result.mPenetrationAxis,
				mPhysicsSettings.mManifoldToleranceSq,
				cast_shape_result.mShape1Face,
				cast_shape_result.mShape2Face,
				manifold.mWorldSpaceContactPointsOn1,
				manifold.mWorldSpaceContactPointsOn2
			);
			...
			if (ccd_body.mContactSettings.mIsSensor)
 			...
 			{
				// Calculate the average position from the manifold (this will result in the same impulse applied as when we apply impulses to all
				// contact points)
...
void PhysicsSystem::JobResolveCCDContacts(PhysicsUpdateContext* ioContext, PhysicsUpdateContext::SubStep* ioSubStep)
{
	...
	if (num_ccd_bodies > 0)
	{
		...
		CCDBody** sorted_ccd_bodies = (CCDBody**)temp_allocator->Allocate(num_ccd_bodies * sizeof(CCDBody*));
		{
			...
			QuickSort(
				sorted_ccd_bodies,
				sorted_ccd_bodies + num_ccd_bodies,
				[](const CCDBody* inBody1, const CCDBody* inBody2)
				{
		...
		for (uint i = 0; i < num_ccd_bodies; ++i)
		{
			...
			if (!ccd_body->mBodyID2.IsInvalid())
			{
				...
				{
					JPH_ASSERT(
						ccd_body2->mBodyID2 != ccd_body->mBodyID1,
						"If we collided with another body, that other body should have ignored collisions with us!"
					);

					// Check if the other body found a hit that is further away
					if (ccd_body2->mFraction > ccd_body->mFraction)
					{
						// Reset the colliding body of the other CCD body. The other body will shorten its distance travelled and will not do any
						// collision response (we'll do that). This means that at this point we have triggered a contact point add/persist for our
						// further hit by accident for the other body. We accept this as calling the contact point callbacks here would require
						// persisting the manifolds up to this point and doing the callbacks single threaded.
						ccd_body2->mBodyID2 = BodyID();
						ccd_body2->mFractionPlusSlop = ccd_body->mFraction;
					}
				}

				// If the other body moved less than us before hitting something, we're not colliding with it so we again have triggered contact
				// point add/persist callbacks by accident. We'll just move to the collision position anyway (as that's the last position we know is
				// good), but we won't do any collision response.
				if (ccd_body2 == nullptr || ccd_body2->mFraction >= ccd_body->mFraction)
				{
					...
					contact_constraint.CalculateConstraintProperties(
						ioContext->mSubStepDeltaTime,
						body1,
						r1_plus_u,
						body2,
						r2,
						ccd_body->mContactNormal,
						normal_velocity_bias
					);
					...
					if (sDrawMotionQualityLinearCast)
					{
						...
						Mat44 collision_transform = Mat44::sTranslation(ccd_body->mFraction * ccd_body->mDeltaPosition) *
						                            body1.GetCenterOfMassTransform();
						...
						Mat44 collision_transform_plus_slop = Mat44::sTranslation(ccd_body->mFractionPlusSlop * ccd_body->mDeltaPosition) *
						                                      body1.GetCenterOfMassTransform();
						body1.GetShape()
							->Draw(DebugRenderer::sInstance, collision_transform_plus_slop, Vec3::sReplicate(1.0f), Color::sOrange, false, true);

						// Draw contact normal
						DebugRenderer::sInstance
							->DrawArrow(ccd_body->mContactPointOn2, ccd_body->mContactPointOn2 - ccd_body->mContactNormal, Color::sYellow, 0.1f);

						// Draw post contact velocity
						DebugRenderer::sInstance->DrawArrow(
							collision_transform.GetTranslation(),
							collision_transform.GetTranslation() + body1.GetLinearVelocity(),
							Color::sOrange,
							0.1f
						);
						DebugRenderer::sInstance->DrawArrow(
							collision_transform.GetTranslation(),
							collision_transform.GetTranslation() + body1.GetAngularVelocity(),
							Color::sPurple,
							0.1f
						);
					}
...
void PhysicsSystem::JobSolvePositionConstraints(PhysicsUpdateContext* ioContext, PhysicsUpdateContext::SubStep* ioSubStep)
{
	...
	for (;;)
	{
		...
		if (has_contacts || has_constraints)
		{
			...
			{
				// In the first iteration also calculate the number of position steps (this way we avoid pulling all constraints into the cache
				// twice)
				bool constraint_impulse = ConstraintManager::sSolvePositionConstraints(
					active_constraints,
					constraints_begin,
					constraints_end,
					delta_time,
...
JPH::Vec3 Body::GetGravity(
	const PhysicsSystem& physicsSystem,
	const BodyLockInterface& bodyLockInterface,
	const JPH::BroadPhaseLayer gravityBroadPhaseLayer,
	const JPH::ObjectLayer gravityObjectLayer
) const
{
	JPH::Vec3 gravity = physicsSystem.GetGlobalGravity();
	const JPH::NarrowPhaseQuery& narrowPhaseQuery = physicsSystem.GetNarrowPhaseQuery();

	// Determine position to test
	const JPH::Vec3 position = GetPosition();
	const JPH::Quat rotation = GetRotation();
	const JPH::Mat44 queryTransform = JPH::Mat44::sRotationTranslation(rotation, position + rotation * mShape->GetCenterOfMass());

	// Settings for collide shape
	JPH::CollideShapeSettings settings;
	settings.mActiveEdgeMode = JPH::EActiveEdgeMode::CollideWithAll;

	JPH::SpecifiedBroadPhaseLayerFilter broadPhaseLayerFilter{gravityBroadPhaseLayer};
	JPH::SpecifiedObjectLayerFilter objectLayerFilter{gravityObjectLayer};
	JPH::IgnoreSingleBodyFilter bodyFilter(GetID());

	struct Collector final : public JPH::CollideShapeCollector
	{
		explicit Collector(const JPH::BodyLockInterface& bodyLockInterface)
			: mBodyLockInterface(bodyLockInterface)
		{
		}
		...
		{
			const Body& body2 = *mBodyLockInterface.TryGetBody(inResult.mBodyID2);
			m_gravity += body2.GetLinearVelocity();
		}

		const JPH::BodyLockInterface& mBodyLockInterface;
		JPH::Vec3 m_gravity{0, 0, 0};
	};

	Collector collector{bodyLockInterface};
