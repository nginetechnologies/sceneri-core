...
{
	// Set inverse mass
	mInvMass = inMassProperties.mMass > 0 ? 1.0f / inMassProperties.mMass : 0.f;
...
void MotionProperties::MoveKinematic(Vec3Arg inDeltaPosition, float inDeltaTime)
{
...
void MotionProperties::MoveKinematic(QuatArg inDeltaRotation, float inDeltaTime)
{
	JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::ReadWrite));
	JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess, BodyAccess::EAccess::Read));
	JPH_ASSERT(mCachedMotionType != EMotionType::Static);
