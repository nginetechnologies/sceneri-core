...
/// Class that provides operations on bodies using a body ID. Note that if you need to do multiple operations on a single body, it is more
/// efficient to lock the body once and combine the operations. All quantities are in world space unless otherwise specified.
...
public:
	...
	Body& CreateBody(const BodyID identifier, const BodyCreationSettings& inSettings);
	...
	/// Adding many bodies, one at a time, results in a really inefficient broadphase until PhysicsSystem::OptimizeBroadPhase is called or
	/// when PhysicsSystem::Update rebuilds the tree! After adding, to get a body by ID use the BodyLockRead or BodyLockWrite interface!
	...
	bool IsBodyValid(const BodyID& inBodyID) const;
	...
	void CreateAndAddBody(const BodyID identifier, const BodyCreationSettings& inSettings, EActivation inActivationMode);
	...
	void NotifyShapeChanged(
		const BodyID& inBodyID, Vec3Arg inPreviousCenterOfMass, bool inUpdateMassProperties, EActivation inActivationMode
	) const;
	///@}
	...
	void SetPositionAndRotationWhenChanged(
		const BodyID& inBodyID, Vec3Arg inPosition, QuatArg inRotation, EActivation inActivationMode
	); ///< Will only update the position/rotation and activate the body when the difference is larger than a very small number. This avoids
	   ///< updating the broadphase/waking up a body when the resulting position/orientation doesn't really change.
	...
	/// Set velocity of body such that it will be positioned at inTargetPosition/Rotation in inDeltaTime seconds (will activate body if
	/// needed)
	void MoveKinematic(const BodyID& inBodyID, Vec3Arg inTargetPosition, float inDeltaTime);
	void MoveKinematic(const BodyID& inBodyID, QuatArg inTargetRotation, float inDeltaTime);
	...
	/// Note that the linear velocity is the velocity of the center of mass, which may not coincide with the position of your object, to
	/// correct for this: \f$VelocityCOM = Velocity - AngularVelocity \times ShapeCOM\f$
	...
	void AddLinearAndAngularVelocity(
		const BodyID& inBodyID, Vec3Arg inLinearVelocity, Vec3Arg inAngularVelocity
	...
	Vec3 GetPointVelocity(
		const BodyID& inBodyID, Vec3Arg inPoint
	) const; ///< Velocity of point inPoint (in world space, e.g. on the surface of the body) of the body

	/// Set the complete motion state of a body.
	/// Note that the linear velocity is the velocity of the center of mass, which may not coincide with the position of your object, to
	/// correct for this: \f$VelocityCOM = Velocity - AngularVelocity \times ShapeCOM\f$
	void SetPositionRotationAndVelocity(
		const BodyID& inBodyID, Vec3Arg inPosition, QuatArg inRotation, Vec3Arg inLinearVelocity, Vec3Arg inAngularVelocity
	);
	...
	void
	AddForceAndTorque(const BodyID& inBodyID, Vec3Arg inForce, Vec3Arg inTorque); ///< A combination of Body::AddForce and Body::AddTorque
	///@}
