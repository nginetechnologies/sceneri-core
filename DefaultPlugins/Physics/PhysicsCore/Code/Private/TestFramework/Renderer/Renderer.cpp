...
namespace JPH::DebugRendering
{
	...
	void Renderer::CreateDepthBuffer()
	{
		...
		FatalErrorIfFailed(mDevice->CreateCommittedResource(
			&heap_properties,
	...
	void Renderer::Initialize()
	{
		...
		mhWnd = CreateWindowW(
			MAKE_NATIVE_LITERAL("TestFrameworkClass"),
			MAKE_NATIVE_LITERAL("TestFramework"),
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
			rc.right - rc.left,
		...
		{
			for (UINT index = 0; DXGI_ERROR_NOT_FOUND !=
			                     factory6->EnumAdapterByGpuPreference(index, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&adapter));
			     ++index)
			{
		...
		FatalErrorIfFailed(
			mDevice
				->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, mCommandAllocators[mFrameIndex].Get(), nullptr, IID_PPV_ARGS(&mCommandList))
		);

		// Command lists are created in the recording state, but there is nothing to record yet. The main loop expects it to be closed, so close
		// it now
	...
	void Renderer::OnWindowResize()
	{
		...
		// Since we may have switched frame index and we know everything is done, we need to update the fence value for our other frame as
		// completed
		for (uint n = 0; n < cFrameCount; ++n)
			if (mFrameIndex != n)
	...
	static inline Mat44 sPerspective(float inFovY, float inAspect, float inNear, float inFar)
	{
		...
		return Mat44(
			...
			Vec4(0.0f, 0.0f, range * inNear, 0.0f)
		);
	}
	...
	void Renderer::BeginFrame(const CameraState& inCamera, float inWorldScale)
	{
		...
		float camera_aspect = static_cast<float>(GetWindowWidth()) / (float)GetWindowHeight();
		...
		vs->mProjection = Mat44(
			Vec4(2.0f / (float)mWindowWidth, 0.0f, 0.0f, 0.0f),
			Vec4(0.0f, -2.0f / (float)mWindowHeight, 0.0f, 0.0f),
	...
	ComPtr<ID3DBlob> Renderer::CreateVertexShader(const ngine::IO::ConstZeroTerminatedPathView filePath)
	{
		...
		Array<uint8> data = ReadData(filePath);

		ngine::String string(filePath.GetView());
	...
	ComPtr<ID3DBlob> Renderer::CreatePixelShader(const ngine::IO::ConstZeroTerminatedPathView filePath)
	{
		UINT flags = D3DCOMPILE_ENABLE_STRICTNESS;
#if DEBUG_BUILD
		flags |= D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION;
	...
	unique_ptr<PipelineState> Renderer::CreatePipelineState(
		ID3DBlob* inVertexShader,
		const D3D12_INPUT_ELEMENT_DESC* inInputDescription,
		uint inInputDescriptionCount,
		ID3DBlob* inPixelShader,
		D3D12_FILL_MODE inFillMode,
		D3D12_PRIMITIVE_TOPOLOGY_TYPE inTopology,
		PipelineState::EDepthTest inDepthTest,
		PipelineState::EBlendMode inBlendMode,
		PipelineState::ECullMode inCullMode
	)
	{
		return make_unique<PipelineState>(
			this,
			inVertexShader,
			inInputDescription,
	...
	ComPtr<ID3D12Resource> Renderer::CreateD3DResource(D3D12_HEAP_TYPE inHeapType, D3D12_RESOURCE_STATES inResourceState, uint64 inSize)
	{
		...
		FatalErrorIfFailed(
			mDevice->CreateCommittedResource(&heap_properties, D3D12_HEAP_FLAG_NONE, &desc, inResourceState, nullptr, IID_PPV_ARGS(&resource))
		);
		return resource;
	}
