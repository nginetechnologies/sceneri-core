...
void QuadTree::GetBodyLocation(const TrackingVector& inTracking, BodyID inBodyID, uint32& outNodeIdx, uint32& outChildIdx) const
{
	...
	JPH_ASSERT(
		mAllocator->Get(outNodeIdx).mChildNodeID[outChildIdx] == inBodyID,
		"Make sure that the body is in the node where it should be"
	);
}
...
void QuadTree::UpdatePrepare(const BodyView inBodies, TrackingVector& ioTracking, UpdateState& outUpdateState, bool inFullRebuild)
{
#ifdef JPH_ENABLE_ASSERTS
	...
	mAllNodeIds.Resize(mNumBodies, ngine::Memory::Uninitialized);
	NodeID* cur_node_id = mAllNodeIds.GetData();
...
#ifdef JPH_ENABLE_ASSERTS
			JPH::BodyID body_id = node_id.GetBodyID();
			JPH_ASSERT(ioTracking[body_id.GetIndex()].mObjectLayer == inBodies[body_id]->GetObjectLayer());
#endif
	...
	uint32 num_node_ids = uint32(cur_node_id - mAllNodeIds.GetData());
	...
	if (num_node_ids > 0)
	{
		...
		// Build new tree
		AABox root_bounds;
		root_node_id = BuildTree(inBodies, ioTracking, mAllNodeIds.GetData(), num_node_ids, cMaxDepthMarkChanged, root_bounds);
...
void QuadTree::UpdateFinalize(
	[[maybe_unused]] const BodyView inBodies, [[maybe_unused]] const TrackingVector& inTracking, const UpdateState& inUpdateState
)
{
...
AABox QuadTree::GetNodeOrBodyBounds(const BodyView inBodies, NodeID inNodeID) const
{
	if (inNodeID.IsNode())
	{
		// It is a node
	...
	{
		// It is a body
		return inBodies[inNodeID.GetBodyID()]->GetWorldSpaceBounds();
	}
}

QuadTree::NodeID QuadTree::BuildTree(
	const BodyView inBodies, TrackingVector& ioTracking, NodeID* ioNodeIDs, int inNumber, uint inMaxDepthMarkChanged, AABox& outBounds
)
{
	...
	mCenters.Resize(inNumber, ngine::Memory::Uninitialized);
	Vec3* centers = mCenters.GetData();
...
bool QuadTree::TryCreateNewRoot(
	TrackingVector& ioTracking, atomic<uint32>& ioRootNodeIndex, NodeID inLeafID, const AABox& inLeafBounds, int inLeafNumBodies
)
{
	// Fetch old root
	...
	// First child is current root, note that since the tree may be modified concurrently we cannot assume that the bounds of our child will
...
void QuadTree::AddBodiesPrepare(const BodyView inBodies, TrackingVector& ioTracking, BodyID* ioBodyIDs, int inNumber, AddState& outState)
{
	// Assert sane input
...
void QuadTree::RemoveBodies([[maybe_unused]] const BodyView inBodies, TrackingVector& ioTracking, const BodyID* ioBodyIDs, int inNumber)
{
	// Assert sane input
	JPH_ASSERT(ioBodyIDs != nullptr);
	JPH_ASSERT(inNumber > 0);
	...
	for (const BodyID *cur = ioBodyIDs, *end = ioBodyIDs + inNumber; cur < end; ++cur)
	{
		...
		JPH_ASSERT(inBodies[*cur]->GetID() == *cur, "Provided BodyID doesn't match BodyID in body manager");
...
void QuadTree::NotifyBodiesAABBChanged(const BodyView inBodies, const TrackingVector& inTracking, const BodyID* ioBodyIDs, int inNumber)
{
	// Assert sane input
	JPH_ASSERT(ioBodyIDs != nullptr);
	JPH_ASSERT(inNumber > 0);
...
JPH_INLINE void QuadTree::WalkTree(
	const ObjectLayerFilter& inObjectLayerFilter,
	const TrackingVector& inTracking,
	Visitor& ioVisitor JPH_IF_TRACK_BROADPHASE_STATS(, LayerToStats& ioStats)
) const
{
	// Get the root
	const RootNode& root_node = GetCurrentRoot();
	...
	do
	{
		...
		if (child_node_id.IsBody())
		{
			...
			ObjectLayer object_layer =
				inTracking[body_id.GetIndex()].mObjectLayer; // We're not taking a lock on the body, so it may be in the process of being removed so
			                                               // check if the object layer is invalid
		...
		else if (child_node_id.IsValid())
		{
			...
			if (top + 4 < cStackSize)
			{
				...
				int num_results =
					ioVisitor.VisitNodes(bounds_minx, bounds_miny, bounds_minz, bounds_maxx, bounds_maxy, bounds_maxz, child_ids, top);
...
void QuadTree::CastRay(
	const RayCast& inRay, RayCastBodyCollector& ioCollector, const ObjectLayerFilter& inObjectLayerFilter, const TrackingVector& inTracking
) const
{
	class Visitor
	{
...
void QuadTree::CollideAABox(
	const AABox& inBox, CollideShapeBodyCollector& ioCollector, const ObjectLayerFilter& inObjectLayerFilter, const TrackingVector& inTracking
) const
{
	class Visitor
	{
...
void QuadTree::CollidePoint(
	Vec3Arg inPoint, CollideShapeBodyCollector& ioCollector, const ObjectLayerFilter& inObjectLayerFilter, const TrackingVector& inTracking
) const
{
	class Visitor
	{
...
void QuadTree::CollideOrientedBox(
...
void QuadTree::CastAABox(
	const AABoxCast& inBox,
...
void QuadTree::FindCollidingPairs(
	const BodyView inBodies,
...
{
	// Note that we don't lock the tree at this point. We know that the tree is not going to be swapped or deleted while finding collision
	// pairs due to the way the jobs are scheduled in the PhysicsSystem::Update. We double check this at the end of the function.
	const RootNode& root_node = GetCurrentRoot();
	...
	for (int b1 = 0; b1 < inNumActiveBodies; ++b1)
	{
		...
		const Body& body1 = *inBodies[b1_id];
		...
		do
		{
			...
			if (child_node_id.IsBody())
			{
				...
				if (b1_id != b2_id)
				{
					...
					const Body& body2 = *inBodies[b2_id];
	...
	// This would mean that UpdateFinalize/DiscardOldTree ran during collision detection which should not be possible due to the way the jobs
	// are scheduled.
	JPH_ASSERT(root_node.mIndex != cInvalidNodeIndex);
...
void QuadTree::ValidateTree(const BodyView inBodies, const TrackingVector& inTracking, uint32 inNodeIndex, uint32 inNumExpectedBodies) const
{
	...
	[[maybe_unused]] uint32 num_bodies = 0;
	...
	do
	{
		...
		JPH_ASSERT(
			cur_stack.mParentNodeIndex == cInvalidNodeIndex || mAllocator->Get(cur_stack.mParentNodeIndex).mIsChanged || !node.mIsChanged
		);
		...
		for (uint32 i = 0; i < 4; ++i)
		{
			...
			if (child_node_id.IsValid())
			{
				...
				else
				{
					...
					const Body* body = inBodies[child_node_id.GetBodyID()];
					[[maybe_unused]] AABox cached_body_bounds = body->GetWorldSpaceBounds();
					[[maybe_unused]] AABox real_body_bounds =
						body->GetShape()->GetWorldSpaceBounds(body->GetCenterOfMassTransform(), Vec3::sReplicate(1.0f));
...
void QuadTree::DumpTree(const NodeID& inRoot, const char* inFileNamePrefix) const
{
	...
	do
	{
		...
		else
		{
			...
			f << "node" << node_str << "[label = \"Node " << node_str << "\nVolume: " << ConvertToString(bounds.GetVolume())
				<< "\" color=" << (node.mIsChanged ? "red" : "black") << "]\n";
...
void QuadTree::ReportStats(const char* inName, const LayerToStats& inLayer) const
{
	...
	for (const LayerToStats::value_type& kv : inLayer)
	{
		...
		double hits_reported_vs_bodies_visited = kv.second.mBodiesVisited > 0
		                                           ? 100.0 * double(kv.second.mHitsReported) / double(kv.second.mBodiesVisited)
		                                           : 100.0;
		double hits_reported_vs_nodes_visited = kv.second.mNodesVisited > 0 ? double(kv.second.mHitsReported) / double(kv.second.mNodesVisited)
		                                                                    : -1.0f;

		stringstream str;
		str << inName << ", " << kv.first << ", " << mName << ", " << kv.second.mNumQueries << ", " << total_time << ", "
				<< total_time_excl_collector << ", " << kv.second.mNodesVisited << ", " << kv.second.mBodiesVisited << ", "
				<< kv.second.mHitsReported << ", " << hits_reported_vs_bodies_visited << ", " << hits_reported_vs_nodes_visited;
		Trace(str.str().c_str());
	}
}
