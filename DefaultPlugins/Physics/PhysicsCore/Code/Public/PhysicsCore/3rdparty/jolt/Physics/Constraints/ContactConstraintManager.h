...
public:
	...
	/// @param inMaxBodyPairs Maximum amount of body pairs to process (anything else will fall through the world), this number should
	/// generally be much higher than the max amount of contact points as there will be lots of bodies close that are not actually touching
	...
	/// Note that when merging manifolds (when PhysicsSettings::mUseManifoldReduction is true) you will only get a callback for the merged
	/// manifold. It is not possible in that case to get all sub shape ID pairs that were colliding, you'll get the first encountered pair.
	using CombineFunction =
		float (*)(const Body& inBody1, const SubShapeID& inSubShapeID1, const Body& inBody2, const SubShapeID& inSubShapeID2);
	...
	{
		mCombineFriction = inCombineFriction;
	}
	...
	{
		mCombineRestitution = inCombineRestitution;
	}
	...
	/// Get a new allocator context for storing contacts. Note that you should call this once and then add multiple contacts using the
	/// context.
	ContactAllocator GetContactAllocator()
	{
		return mCache[mCacheWriteIdx].GetContactAllocator();
	}
	...
	void GetContactsFromCache(
		ContactAllocator& ioContactAllocator, Body& inBody1, Body& inBody2, bool& outPairHandled, bool& outConstraintCreated
	);
	...
	///
	/// This is using the approach described in 'Modeling and Solving Constraints' by Erin Catto presented at GDC 2009 (and later years with
	/// slight modifications). We're using the formulas from slide 50 - 53 combined.
	///
	...
	bool AddContactConstraint(
		ContactAllocator& ioContactAllocator, BodyPairHandle inBodyPair, Body& inBody1, Body& inBody2, const ContactManifold& inManifold
	);
	...
	/// inExpectedNumBodyPairs / inExpectedNumManifolds are the amount of body pairs / manifolds found in the previous step and is used to
	/// determine the amount of buckets the contact cache hash map will use.
	...
	{
		return min<uint32>(mNumConstraints, mMaxConstraints);
	}

	/// Sort contact constraints deterministically
	void SortContacts(uint32* inConstraintIdxBegin, uint32* inConstraintIdxEnd) const;

	/// AddContactConstraint will also setup the velocity constraints for the first sub step. For subsequent sub steps this function must be
	/// called prior to warm starting the constraint.
...
private:
	...
	public:
		...
		static int sGetRequiredExtraSize(int inNumContactPoints)
		{
			return max(0, inNumContactPoints - 1) * sizeof(CachedContactPoint);
		}

		/// Calculate total class size needed for storing inNumContactPoints
		static int sGetRequiredTotalSize(int inNumContactPoints)
		{
		...
		enum class EFlags : uint16
		{
			...
			CCDContact =
				2 ///< This is a cached manifold reported by continuous collision detection and was only used to create a contact callback
		};
	...
	private:
		...
		/// Allocator used by both mCachedManifolds and mCachedBodyPairs, this makes it more likely that a body pair and its manifolds are close
		/// in memory
	...
	public:
		...
		void CalculateNonPenetrationConstraintProperties(
			...
			const Body& inBody1,
			const Body& inBody2,
			Vec3Arg inWorldSpacePosition1,
			Vec3Arg inWorldSpacePosition2,
			Vec3Arg inWorldSpaceNormal
		);
		template<EMotionType Type1, EMotionType Type2>
		JPH_INLINE void CalculateFrictionAndNonPenetrationConstraintProperties(
			float inDeltaTime,
			const Body& inBody1,
			const Body& inBody2,
			Mat44Arg inInvI1,
	...
	/// Internal helper function to calculate the friction and non-penetration constraint properties. Templated to the motion type to reduce
	/// the amount of branches and calculations.
	template<EMotionType Type1, EMotionType Type2>
	JPH_INLINE void TemplatedCalculateFrictionAndNonPenetrationConstraintProperties(
		ContactConstraint& ioConstraint,
		float inDeltaTime,
		Mat44Arg inTransformBody1,
		Mat44Arg inTransformBody2,
		const Body& inBody1,
		const Body& inBody2,
		Mat44Arg inInvI1,
		Mat44Arg inInvI2
	...
	inline void CalculateFrictionAndNonPenetrationConstraintProperties(
		ContactConstraint& ioConstraint,
		float inDeltaTime,
		Mat44Arg inTransformBody1,
		Mat44Arg inTransformBody2,
	...
	bool TemplatedAddContactConstraint(
		ContactAllocator& ioContactAllocator,
		BodyPairHandle inBodyPairHandle,
		Body& inBody1,
		Body& inBody2,
		const ContactManifold& inManifold,
		Mat44Arg inInvI1,
	...
	/// Internal helper function to warm start contact constraint. Templated to the motion type to reduce the amount of branches and
	/// calculations.
	template<EMotionType Type1, EMotionType Type2>
	JPH_INLINE static void sWarmStartConstraint(
		ContactConstraint& ioConstraint,
	...
	/// Internal helper function to solve a single contact constraint. Templated to the motion type to reduce the amount of branches and
	/// calculations.
	template<EMotionType Type1, EMotionType Type2>
	JPH_INLINE static bool
	sSolveVelocityConstraint(ContactConstraint& ioConstraint, MotionProperties* ioMotionProperties1, MotionProperties* ioMotionProperties2);
	...
	CombineFunction mCombineFriction = [](const Body& inBody1, const SubShapeID& subShape1, const Body& inBody2, const SubShapeID& subShape2)
	{
		const float friction1 = inBody1.GetShape()->GetMaterial(subShape1)->GetFriction();
		const float friction2 = inBody2.GetShape()->GetMaterial(subShape2)->GetFriction();
		return sqrt(friction1 * friction2);
	};
	CombineFunction mCombineRestitution =
		[](const Body& inBody1, const SubShapeID& subShape1, const Body& inBody2, const SubShapeID& subShape2)
	{
		const float restitution1 = inBody1.GetShape()->GetMaterial(subShape1)->GetRestitution();
		const float restitution2 = inBody2.GetShape()->GetMaterial(subShape2)->GetRestitution();
