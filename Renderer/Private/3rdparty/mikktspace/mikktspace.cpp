...
PUSH_MSVC_WARNINGS
DISABLE_MSVC_WARNINGS(4324 4310 4458 4100 4296 4701 4456)
...
static int
GenerateInitialVerticesIndexList(STriInfo pTriInfos[], int piTriList_out[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn);
...
static int
Build4RuleGroups(STriInfo pTriInfos[], SGroup pGroups[], int piGroupTrianglesBuffer[], const int piTriListIn[], const int iNrTrianglesIn);
...
tbool genTangSpace(const SMikkTSpaceContext* pContext, const float fAngularThreshold)
{
	...
	piTriListIn = (int*)ngine::Memory::Allocate(sizeof(int) * 3 * iNrTrianglesIn);
	pTriInfos = (STriInfo*)ngine::Memory::Allocate(sizeof(STriInfo) * iNrTrianglesIn);
	if (piTriListIn == NULL || pTriInfos == NULL)
	{
		if (piTriListIn != NULL)
			ngine::Memory::Deallocate(piTriListIn);
		if (pTriInfos != NULL)
			ngine::Memory::Deallocate(pTriInfos);
		return TFALSE;
	}
	...
	pGroups = (SGroup*)ngine::Memory::Allocate(sizeof(SGroup) * iNrMaxGroups);
	piGroupTrianglesBuffer = (int*)ngine::Memory::Allocate(sizeof(int) * iNrTrianglesIn * 3);
	if (pGroups == NULL || piGroupTrianglesBuffer == NULL)
	{
		if (pGroups != NULL)
			ngine::Memory::Deallocate(pGroups);
		if (piGroupTrianglesBuffer != NULL)
			ngine::Memory::Deallocate(piGroupTrianglesBuffer);
		ngine::Memory::Deallocate(piTriListIn);
		ngine::Memory::Deallocate(pTriInfos);
		return TFALSE;
	}
	...
	psTspace = (STSpace*)ngine::Memory::Allocate(sizeof(STSpace) * iNrTSPaces);
	if (psTspace == NULL)
	{
		ngine::Memory::Deallocate(piTriListIn);
		ngine::Memory::Deallocate(pTriInfos);
	...
	if (!bRes) // if an allocation in GenerateTSpaces() failed
	{
		...
		ngine::Memory::Deallocate(pTriInfos);
		ngine::Memory::Deallocate(piTriListIn);
		ngine::Memory::Deallocate(psTspace);
		return TFALSE;
	}
...
static void
MergeVertsFast(int piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext* pContext, const int iL_in, const int iR_in);
...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
	...
	// make allocations
	piHashTable = (int*)ngine::Memory::Allocate(sizeof(int) * iNrTrianglesIn * 3);
	piHashCount = (int*)ngine::Memory::Allocate(sizeof(int) * g_iCells);
	piHashOffsets = (int*)ngine::Memory::Allocate(sizeof(int) * g_iCells);
	piHashCount2 = (int*)ngine::Memory::Allocate(sizeof(int) * g_iCells);

	if (piHashTable == NULL || piHashCount == NULL || piHashOffsets == NULL || piHashCount2 == NULL)
	{
		if (piHashTable != NULL)
			ngine::Memory::Deallocate(piHashTable);
		if (piHashCount != NULL)
			ngine::Memory::Deallocate(piHashCount);
		if (piHashOffsets != NULL)
			ngine::Memory::Deallocate(piHashOffsets);
		if (piHashCount2 != NULL)
			ngine::Memory::Deallocate(piHashCount2);
	...
	pTmpVert = (STmpVert*)ngine::Memory::Allocate(sizeof(STmpVert) * iMaxCount);
	...
	if (pTmpVert != NULL)
	{
		ngine::Memory::Deallocate(pTmpVert);
	}
	ngine::Memory::Deallocate(piHashTable);
	ngine::Memory::Deallocate(piHashCount);
	ngine::Memory::Deallocate(piHashOffsets);
		...
		else
		{
...

			{
				...
				else
				{
					...
					const float distSQ_02P = LengthSquared(vsub(P2, P0));
					const float distSQ_13P = LengthSquared(vsub(P3, P1));

					bQuadDiagIs_02 = distSQ_13P < distSQ_02P ? TFALSE : TTRUE;
				}
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
	...
	// match up edge pairs
	{
		SEdge* pEdges = (SEdge*)ngine::Memory::Allocate(sizeof(SEdge) * iNrTrianglesIn * 3);
		if (pEdges == NULL)
		...
		else
		{
			...
			ngine::Memory::Deallocate(pEdges);
		}
	}
}
...
)
{
	...
	[[maybe_unused]] int iMaxNrFaces = 0, iUniqueTspaces = 0, g = 0, i = 0;
	...
	pSubGroupTspace = (STSpace*)ngine::Memory::Allocate(sizeof(STSpace) * iMaxNrFaces);
	pUniSubGroups = (SSubGroup*)ngine::Memory::Allocate(sizeof(SSubGroup) * iMaxNrFaces);
	pTmpMembers = (int*)ngine::Memory::Allocate(sizeof(int) * iMaxNrFaces);
	if (pSubGroupTspace == NULL || pUniSubGroups == NULL || pTmpMembers == NULL)
	{
		if (pSubGroupTspace != NULL)
			ngine::Memory::Deallocate(pSubGroupTspace);
		if (pUniSubGroups != NULL)
			ngine::Memory::Deallocate(pUniSubGroups);
		if (pTmpMembers != NULL)
			ngine::Memory::Deallocate(pTmpMembers);
		return TFALSE;
	}
	...
	for (g = 0; g < iNrActiveGroups; g++)
	{
		...
		for (i = 0; i < pGroup->iNrFaces; i++) // triangles
		{
			...
			{
				// insert new subgroup
				int* pIndices = (int*)ngine::Memory::Allocate(sizeof(int) * iMembers);
				if (pIndices == NULL)
				{
					// clean up and return false
					for (int s = 0; s < iUniqueSubGroups; s++)
						ngine::Memory::Deallocate(pUniSubGroups[s].pTriMembers);
					ngine::Memory::Deallocate(pUniSubGroups);
					ngine::Memory::Deallocate(pTmpMembers);
					ngine::Memory::Deallocate(pSubGroupTspace);
					return TFALSE;
...
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
	...
	while (t < iNrTrianglesIn && bStillFindingGoodOnes)
	{
		...
		else
		{
			...
			{
				const tbool bIsGood2 = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag & MARK_DEGENERATE) == 0 ? TTRUE : TFALSE;
				if (bIsGood2)
