...
MeshShape::MeshShape(const MeshShape& other)
	: Shape(EShapeType::Mesh, EShapeSubType::Mesh)
...
MassProperties MeshShape::GetMassProperties([[maybe_unused]] const float density) const
{
	// sceneri change: Return a non-zero mass so we don't divide by zero elsewhere
	// Mesh shapes can't be dynamic so it shouldn't matter.
	MassProperties massProperties;
	massProperties.mMass = 0.01f;
	return massProperties;
}

MassProperties MeshShape::GetMassProperties() const
{
	// ngine change: Return a non-zero mass so we don't divide by zero elsewhere
	// Mesh shapes can't be dynamic so it shouldn't matter.
	MassProperties massProperties;
	massProperties.mMass = 0.01f;
	return massProperties;
...
void MeshShape::SetMaterial(const SubShapeID& inSubShapeID, const PhysicsMaterial* pMaterial)
{
	const uint materialIndex = GetMaterialIndex(inSubShapeID);
	if (materialIndex < mMaterials.size())
	{
		mMaterials[materialIndex] = pMaterial;
	}
}
