...
void ConvexShape::sCollideConvexVsConvex(
...
)
{
	...
	// Scope to limit lifetime of SupportBuffer
	{
		...
		const Support* shape1_excl_cvx_radius =
			shape1->GetSupportFunction(ConvexShape::ESupportMode::ExcludeConvexRadius, buffer1_excl_cvx_radius, inScale1);
		const Support* shape2_excl_cvx_radius =
			shape2->GetSupportFunction(ConvexShape::ESupportMode::ExcludeConvexRadius, buffer2_excl_cvx_radius, inScale2);
		...
		// Perform GJK step
		status = pen_depth.GetPenetrationDepthStepGJK(
			*shape1_excl_cvx_radius,
			shape1_excl_cvx_radius->GetConvexRadius() + inCollideShapeSettings.mMaxSeparationDistance,
			transformed2_excl_cvx_radius,
			shape2_excl_cvx_radius->GetConvexRadius(),
			inCollideShapeSettings.mCollisionTolerance,
			penetration_axis,
			point1,
			point2
		);
	...
	switch (status)
	{
		...
		case EPAPenetrationDepth::EStatus::Indeterminate:
		{
			...
			const Support* shape1_incl_cvx_radius =
				shape1->GetSupportFunction(ConvexShape::ESupportMode::IncludeConvexRadius, buffer1_incl_cvx_radius, inScale1);
			const Support* shape2_incl_cvx_radius =
				shape2->GetSupportFunction(ConvexShape::ESupportMode::IncludeConvexRadius, buffer2_incl_cvx_radius, inScale2);
			...
			// Perform EPA step
			if (!pen_depth.GetPenetrationDepthStepEPA(
						shape1_add_max_separation_distance,
						transformed2_incl_cvx_radius,
						inCollideShapeSettings.mPenetrationTolerance,
						penetration_axis,
						point1,
						point2
					))
	...
	CollideShapeResult result(
		...
		TransformedShape::sGetBodyID(ioCollector.GetContext())
	);
	...
	if (CastRay(inRay, inSubShapeIDCreator, hit))
	{
		...
		if (inRayCastSettings.mBackFaceMode == EBackFaceMode::CollideWithBackFaces && !ioCollector.ShouldEarlyOut())
		{
			...
			if (delta_fraction < 0.0f)
			{
				...
				if (CastRay(inverted_ray, inSubShapeIDCreator, inverted_hit) && inverted_hit.mFraction > 0.0f) // Ignore hits with fraction 0, this
				                                                                                               // means the ray ends inside the
				                                                                                               // object and we don't want to report
				                                                                                               // it as a back facing hit
				{
...
)
{
	...
	ConvexShape::ESupportMode support_mode = inShapeCastSettings.mUseShrunkenShapeAndConvexRadius
	                                           ? ConvexShape::ESupportMode::ExcludeConvexRadius
	                                           : ConvexShape::ESupportMode::IncludeConvexRadius;
...
void ConvexShape::GetTrianglesStart(
...
int ConvexShape::GetTrianglesNext(
...
void ConvexShape::GetSubmergedVolume(
...
) const
{
	...
	using Face = int[5];
#define MAKE_FACE(a, b, c, d) \
	{a, b, c, d, ((1 << a) | (1 << b) | (1 << c) | (1 << d))} // Last int is a bit mask that indicates which indices are used
	Face faces[] = {
		MAKE_FACE(0, 2, 3, 1),
		MAKE_FACE(4, 6, 2, 0),
		MAKE_FACE(4, 5, 7, 6),
	...
	PolyhedronSubmergedVolumeCalculator::Point* buffer = (PolyhedronSubmergedVolumeCalculator::Point*)
		JPH_STACK_ALLOC(8 * sizeof(PolyhedronSubmergedVolumeCalculator::Point));
	PolyhedronSubmergedVolumeCalculator
		submerged_vol_calc(inCenterOfMassTransform * Mat44::sScale(extent), points, sizeof(Vec3), 8, inSurface, buffer);
...
void ConvexShape::DrawGetSupportFunction(
...
) const
{
	...
	// Draw the shape
	DebugRenderer::GeometryRef geometry = inRenderer->CreateTriangleGeometryForConvex(
		[&add_convex](Vec3Arg inDirection)
		{
	...
	{
		// Iterate on all directions and draw the support point and an arrow in the direction that was sampled to test if the support points
		// make sense
