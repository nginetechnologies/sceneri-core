...
/// The Body class only keeps track of state for static bodies, the MotionProperties class keeps the additional state needed for a moving
/// Body. It has a 1-on-1 relationship with the body.
...
public:
	...
	EMotionQuality GetMotionQuality() const
	{
		return mMotionQuality;
	}
	void SetMotionQuality(EMotionQuality inQuality)
	{
		mMotionQuality = inQuality;
	}

	/// Get world space linear velocity of the center of mass
	inline Vec3 GetLinearVelocity() const
	{
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::Read));
	...
	void SetLinearVelocity(Vec3Arg inLinearVelocity)
	{
		...
		mLinearVelocity = inLinearVelocity;
	}
	...
	inline void MoveKinematic(Vec3Arg inDeltaPosition, float inDeltaTime);
	inline void MoveKinematic(QuatArg inDeltaRotation, float inDeltaTime);
	...
	inline float GetMaxLinearVelocity() const
	{
		return mMaxLinearVelocity;
	}
	inline void SetMaxLinearVelocity(float inLinearVelocity)
	{
		JPH_ASSERT(inLinearVelocity >= 0.0f);
		mMaxLinearVelocity = inLinearVelocity;
	}
	...
	inline void SetMaxAngularVelocity(float inAngularVelocity)
	{
		JPH_ASSERT(inAngularVelocity >= 0.0f);
		mMaxAngularVelocity = inAngularVelocity;
	}
	///@}
	...
	{
		JPH_ASSERT(inLinearDamping >= 0.0f);
		mLinearDamping = inLinearDamping;
	}
	...
	{
		JPH_ASSERT(inAngularDamping >= 0.0f);
		mAngularDamping = inAngularDamping;
	}
	...
	{
		mGravityFactor = inGravityFactor;
	}

	/// Set the mass and inertia tensor
	inline void SetMassProperties(const MassProperties& inMassProperties);

	/// Get inverse mass (1 / mass). Should only be called on a dynamic object (static or kinematic bodies have infinite mass so should be
	/// treated as 1 / mass = 0)
	...
	inline float GetInverseMassUnchecked() const
	{
		return mInvMass;
	}
	...
	{
		mInvMass = inInverseMass;
	}

	/// Diagonal of inverse inertia matrix: D. Should only be called on a dynamic object (static or kinematic bodies have infinite mass so
	/// should be treated as D = 0)
	...
	inline Quat GetInertiaRotation() const
	{
		return mInertiaRotation;
	}
	...
	/// Get inverse inertia matrix (\f$I^{-1}\f$) for a given object rotation (translation will be ignored). Zero if object is static or
	/// kinematic.
	...
	///@{
	inline void AddLinearVelocityStep(Vec3Arg inLinearVelocityChange)
	{
		JPH_DET_LOG("AddLinearVelocityStep: " << inLinearVelocityChange);
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::ReadWrite));
		mLinearVelocity += inLinearVelocityChange;
		JPH_ASSERT(!mLinearVelocity.IsNaN());
	}
	inline void SubLinearVelocityStep(Vec3Arg inLinearVelocityChange)
	{
		JPH_DET_LOG("SubLinearVelocityStep: " << inLinearVelocityChange);
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::ReadWrite));
		mLinearVelocity -= inLinearVelocityChange;
		JPH_ASSERT(!mLinearVelocity.IsNaN());
	}
	inline void AddAngularVelocityStep(Vec3Arg inAngularVelocityChange)
	{
		JPH_DET_LOG("AddAngularVelocityStep: " << inAngularVelocityChange);
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::ReadWrite));
		mAngularVelocity += inAngularVelocityChange;
		JPH_ASSERT(!mAngularVelocity.IsNaN());
	}
	inline void SubAngularVelocityStep(Vec3Arg inAngularVelocityChange)
	{
		JPH_DET_LOG("SubAngularVelocityStep: " << inAngularVelocityChange);
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess, BodyAccess::EAccess::ReadWrite));
		mAngularVelocity -= inAngularVelocityChange;
		JPH_ASSERT(!mAngularVelocity.IsNaN());
	}
	...
	inline void ResetForceAndTorqueInternal()
	{
		mForce = Float3(0, 0, 0);
		mTorque = Float3(0, 0, 0);
	}
...
private:
	...
	// 4 byte aligned
	Float3 mForce{
		0, 0, 0}; ///< Accumulated world space force (N). Note loaded through intrinsics so ensure that the 4 bytes after this are readable!
	Float3 mTorque{
		0, 0, 0}; ///< Accumulated world space torque (N m). Note loaded through intrinsics so ensure that the 4 bytes after this are readable!
	...
	uint32 mIslandIndex; ///< Index of the island that this body is part of, when the body has not yet been updated or is not active this is
	                     ///< cInactiveIndex
	...
	// 4 byte aligned
	Sphere mSleepTestSpheres[3]; ///< Measure motion for 3 points on the body to see if it is resting: COM, COM + largest bounding box axis,
	                             ///< COM + second largest bounding box axis
