...
///
/// Note that internally all properties (position, velocity etc.) are tracked relative to the center of mass of the object to simplify the
...
/// The functions that get/set the position of the body all indicate if they are relative to the center of mass or to the original position
/// in which the shape was created.
///
/// The linear velocity is also velocity of the center of mass, to correct for this: \f$VelocityCOM = Velocity - AngularVelocity \times
/// ShapeCOM\f$.
...
public:
	...
	~Body()
	{
		JPH_ASSERT(mMotionProperties == nullptr);
	}

	/// Get the id of this body
	inline const BodyID& GetID() const
	{
		return mID;
	}
	...
	{
		return mMotionProperties != nullptr && mMotionProperties->mIndexInActiveBodies != cInactiveIndex;
	}
	...
	/// Check if this body is kinematic (keyframed), which means that it will move according to its current velocity, but forces don't affect
	/// it
	...
	inline bool CanBeKinematicOrDynamic() const
	{
		return mMotionProperties != nullptr;
	}

	/// Change the body to a sensor. A sensor will receive collision callbacks, but will not cause any collision responses and can be used as
	/// a trigger volume. The cheapest sensor (in terms of CPU usage) is a sensor with motion type Static (they can be moved around using
	/// BodyInterface::SetPosition/SetPositionAndRotation). These sensors will only detect collisions with active Dynamic or Kinematic bodies.
	/// As soon as a body go to sleep, the contact point with the sensor will be lost. If you make a sensor Dynamic or Kinematic and activate
	/// them, the sensor will be able to detect collisions with sleeping bodies too. An active sensor will never go to sleep automatically.
	/// When you make a Dynamic or Kinematic sensor, make sure it is in an ObjectLayer that does not collide with Static bodies or other
	/// sensors to avoid extra overhead in the broad phase.
	inline void SetIsSensor(bool inIsSensor)
	{
		if (inIsSensor)
			mFlags.fetch_or(uint8(EFlags::IsSensor), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::IsSensor)), memory_order_relaxed);
	}

	/// Check if this body is a sensor.
	inline bool IsSensor() const
	{
		return (mFlags.load(memory_order_relaxed) & uint8(EFlags::IsSensor)) != 0;
	}

	/// Motion type of this body
	inline EMotionType GetMotionType() const
	{
		return mMotionType;
	}
	...
	inline BroadPhaseLayer GetBroadPhaseLayer() const
	{
		return mBroadPhaseLayer;
	}
	...
	{
		return !IsStatic() ? mMotionProperties->GetLinearVelocity() : Vec3::sZero();
	}
	...
	{
		return !IsStatic() ? mMotionProperties->GetAngularVelocity() : Vec3::sZero();
	}
	...
	inline Vec3 GetPointVelocityCOM(Vec3Arg inPointRelativeToCOM) const
	{
		return !IsStatic() ? mMotionProperties->GetPointVelocityCOM(inPointRelativeToCOM) : Vec3::sZero();
	}

	/// Velocity of point inPoint (in world space, e.g. on the surface of the body) of the body (unit: m/s)
	inline Vec3 GetPointVelocity(Vec3Arg inPoint) const
	{
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess, BodyAccess::EAccess::Read));
		return GetPointVelocityCOM(inPoint - mPosition);
	}

	/// Add force (unit: N) at center of mass for the next time step, will be reset after the next call to PhysicsSimulation::Update
	inline void AddForce(Vec3Arg inForce)
	{
		JPH_ASSERT(IsDynamic());
		(Vec3::sLoadFloat3Unsafe(mMotionProperties->mForce) + inForce).StoreFloat3(&mMotionProperties->mForce);
	}
	...
	inline void AddTorque(Vec3Arg inTorque)
	{
		JPH_ASSERT(IsDynamic());
		(Vec3::sLoadFloat3Unsafe(mMotionProperties->mTorque) + inTorque).StoreFloat3(&mMotionProperties->mTorque);
	}

	[[nodiscard]] JPH::Vec3 GetGravity(
		const PhysicsSystem&,
	...
	void MoveKinematic(Vec3Arg inTargetPosition, float inDeltaTime);
	void MoveKinematic(QuatArg inTargetRotation, float inDeltaTime);
	...
	/// @param inBuoyancy The buoyancy factor for the body. 1 = neutral body, < 1 sinks, > 1 floats. Note that we don't use the fluid density
	/// since it is harder to configure than a simple number between [0, 2]
	...
	inline bool IsInBroadPhase() const
	{
		return (mFlags.load(memory_order_relaxed) & uint8(EFlags::IsInBroadPhase)) != 0;
	}

	/// Check if this body has been changed in such a way that the collision cache should be considered invalid for any body interacting with
	/// this body
	inline bool IsCollisionCacheInvalid() const
	{
		return (mFlags.load(memory_order_relaxed) & uint8(EFlags::InvalidateContactCache)) != 0;
	}
	...
	inline const AABox& GetWorldSpaceBounds() const
	{
		return mBounds;
	}
	...
	/// It assumes that body 1 is dynamic and active and guarantees that it body 1 collides with body 2 that body 2 will not collide with body
	/// 1 in order to avoid finding duplicate collision pairs
	...
	inline void AddPositionStep(Vec3Arg inLinearVelocityTimesDeltaTime)
	{
		...
		mPosition += inLinearVelocityTimesDeltaTime;
	...
	inline void SetInBroadPhaseInternal(bool inInBroadPhase)
	{
		if (inInBroadPhase)
			mFlags.fetch_or(uint8(EFlags::IsInBroadPhase), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::IsInBroadPhase)), memory_order_relaxed);
	}

	/// Invalidate the contact cache (should only be called by the BodyManager), will be reset the next simulation step. Returns true if the
	/// contact cache was still valid.
	inline bool InvalidateContactCacheInternal()
	{
		return (mFlags.fetch_or(uint8(EFlags::InvalidateContactCache), memory_order_relaxed) & uint8(EFlags::InvalidateContactCache)) == 0;
	}

	/// Reset the collision cache invalid flag (should only be called by the BodyManager).
	inline void ValidateContactCacheInternal()
	{
		JPH_IF_ENABLE_ASSERTS(uint8 old_val =) mFlags.fetch_and(uint8(~uint8(EFlags::InvalidateContactCache)), memory_order_relaxed);
		JPH_ASSERT((old_val & uint8(EFlags::InvalidateContactCache)) != 0);
	}
	...
	/// Updates the center of mass and optionally mass propertes after shifting the center of mass or changes to the shape (should only be
	/// called by the BodyInterface since it also requires updating the broadphase)
...
private:
	...
	inline void GetSleepTestPoints(Vec3* outPoints) const; ///< Determine points to test for checking if body is sleeping: COM, COM + largest
	                                                       ///< bounding box axis, COM + second largest bounding box axis
	inline void ResetSleepTestSpheres();                   ///< Reset spheres to current position as returned by GetSleepTestPoints

	enum class EFlags : uint8
	{
		IsSensor = 1 << 0,       ///< If this object is a sensor. A sensor will receive collision callbacks, but will not cause any collision
		                         ///< responses and can be used as a trigger volume.
		IsInBroadPhase = 1 << 1, ///< Set this bit to indicate that the body is in the broadphase
		InvalidateContactCache =
			1 << 2 ///< Set this bit to indicate that all collision caches for this body are invalid, will be reset the next simulation step.
	};
	...
	MotionProperties* mMotionProperties =
		nullptr;                      ///< If this is a keyframed or dynamic object, this object holds all information about the movement
