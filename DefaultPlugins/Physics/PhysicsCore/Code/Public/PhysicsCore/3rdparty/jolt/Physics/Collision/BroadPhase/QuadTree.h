...
public:
	...
	void						UpdatePrepare(const BodyView inBodies, TrackingVector &ioTracking, UpdateState &outUpdateState, bool inFullRebuild);
	void						UpdateFinalize(const BodyView inBodies, const TrackingVector &inTracking, const UpdateState &inUpdateState);
	...
	void						AddBodiesPrepare(const BodyView inBodies, TrackingVector &ioTracking, BodyID *ioBodyIDs, int inNumber, AddState &outState);
	...
	void						RemoveBodies(const BodyView inBodies, TrackingVector &ioTracking, const BodyID *ioBodyIDs, int inNumber);

	/// Call whenever the aabb of a body changes.
	void						NotifyBodiesAABBChanged(const BodyView inBodies, const TrackingVector &inTracking, const BodyID *ioBodyIDs, int inNumber);
	...
	void						FindCollidingPairs(const BodyView inBodies, const BodyID *inActiveBodies, int inNumActiveBodies, float inSpeculativeContactDistance, BodyPairCollector &ioPairCollector, ObjectLayerPairFilter inObjectLayerPairFilter) const;
...
private:
	...
	inline AABox				GetNodeOrBodyBounds(const BodyView inBodies, NodeID inNodeID) const;
	...
	NodeID						BuildTree(const BodyView inBodies, TrackingVector &ioTracking, NodeID *ioNodeIDs, int inNumber, uint inMaxDepthMarkChanged, AABox &outBounds);
	...
	void						ValidateTree(const BodyView inBodies, const TrackingVector &inTracking, uint32 inNodeIndex, uint32 inNumExpectedBodies) const;
#endif
	...
	ngine::InlineVector<NodeID, 64> mAllNodeIds;
	ngine::InlineVector<Vec3, 32> mCenters;
};
