...
public:
	...
	explicit constexpr Color(uint32 inColor)
		: mU32(inColor)
	{
	}
	...
	/// Comparison
	inline bool operator==(ColorArg inRHS) const
	{
		return mU32 == inRHS.mU32;
	}
	inline bool operator!=(ColorArg inRHS) const
	{
		return mU32 != inRHS.mU32;
	...
	inline uint8 operator()(uint inIdx) const
	{
		JPH_ASSERT(inIdx < 4);
		return (&r)[inIdx];
	}
	inline uint8& operator()(uint inIdx)
	{
		JPH_ASSERT(inIdx < 4);
		return (&r)[inIdx];
	}
	...
	inline uint8 GetIntensity() const
	{
		return uint8((uint32(r) * 54 + g * 183 + b * 19) >> 8);
	}
